...
Partial Class Pages_Roster_Employee_Attendance
...
#Region "Panel Edit"
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        If ValidateInsert() = True Then
            ...
            sql = "Exec sp_tms_calEmpOTLate N'" & mySetting.GetEmployeeIDbyCodeName_ReturnString(Session("Company").ToString, GetStoredProcParameter("EMPLOYEE_PROFILE_ID")) & "',N'" & _
    ...
    Protected Sub imgBtnUpdate_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdate.Click
        ...
        If ValidateUpdate() Then
            ...
            For Each de In CType(Session("HTB"), Hashtable)
                ...
                End If
                If de.Key.ToString.ToUpper = "DATE_TIME_IN" Then
                ...
                End If
                If de.Key.ToString.ToUpper = "DATE_TIME_OUT" Then
                    If de.Value.ToString.Trim <> "" Then
        ...
        If ActionValidateDelete() Then
            ...
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                If chkDelete.Checked Then
                    ...
                    If ssql <> Nothing Then
                        For j As Integer = 0 To gvHistory.HeaderRow.Cells.Count - 1
                            If strParamEmp = gvHistory.HeaderRow.Cells(j).Text.ToString Then
                                strEmpID = myGridView.Rows(i).Cells(j).Text.ToString.Replace("&amp;", "&")
                            End If
                            If strParamDate = gvHistory.HeaderRow.Cells(j).Text.ToString Then
                                strDate = myGridView.Rows(i).Cells(j).Text.ToString.Replace("&amp;", "&")
                            End If
                        Next
                        htb.Add("ssqlDelMaintenance" & i.ToString, "Delete From Employee_OT_Maintenance Where Employee_Profile_ID=dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & strEmpID & "') And Date='" & mySetting.ConvertDateToDecimal(strDate, Session("Company").ToString, Session("Module").ToString) & "'")
                        htb.Add("ssqlDelOTApprovedRecord" & i.ToString, "Delete From OT_Approved_Record Where Company_Profile_Code=N'" & Session("Company").ToString & "' And Employee_Profile_ID=dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & strEmpID & "') And Date='" & mySetting.ConvertDateToDecimal(strDate, Session("Company").ToString, Session("Module").ToString) & "'")
...
#Region "Sub & Function"
    ...
    Private Function ValidateInsert() As Boolean
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ...
                                If myDS.Tables(0).Rows(i).Item(0).ToString.ToUpper = "EMPLOYEE_PROFILE_ID" Then
                                    ssql2 = ssql2 & "ISNULL(dbo.fn_ReturnEmpIDByCodeName('" & Session("Company").ToString & "','" & myTextBox.Text.ToString.Replace("'", "''") & "'),''),"
                                    ssql3 = ssql3 & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myTextBox.Text.ToString.Replace("'", "''") & "'),'') And "
                                    htb.Add(myDS.Tables(0).Rows(i).Item(0).ToString.ToUpper, "'" & Trim(myTextBox.Text) & "'")
                                Else
                                    Dim myDST As New DataSet
                                    myDST = mySQL.ExecuteSQL("Select Function_Name,Default_Value From User_Define_Function Where Company_Profile_Code='" & Session("Company").ToString & "' And Module_Profile_Code='" & Session("Module").ToString & "' And Table_Profile_Code='" & Form.ID & "' And Table_Field_Code='" & myDS.Tables(0).Rows(i).Item(0).ToString & "' And Query_Action='INSERT'")
        ...
        DiffMin = mySQL.fn_DateDiff("minute", mySetting.UnDisplayDateTime(txtDATE_TIME_IN.Text, Session("Company"), Session("Module")), mySetting.UnDisplayDateTime(txtDATE_TIME_OUT.Text, Session("Company"), Session("Module")))

        If DiffMin < 1 Then
            lblresult2.Text = "Time Out Must Bigger Than Time In!"
            txtDATE_TIME_OUT.Focus()
            Return False
...
#End Region
    ...
    '    imgbtnDATE.Attributes.Remove("onclick")
    '    mySetting.PopUpCalendar_ImageButton(imgbtnDATE, Form.ID, txtDATE.ID, txtEMPLOYEE_PROFILE_ID.Text.ToString, txtOCP_ID_TMS.Text.ToString)
    'End Sub

    'Protected Sub imgbtnOCP_ID_TMS_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnOCP_ID_TMS.Click
    '    imgbtnDATE.Attributes.Remove("onclick")
    '    mySetting.PopUpCalendar_ImageButton(imgbtnDATE, Form.ID, txtDATE.ID, txtOCP_ID_TMS.Text.ToString)
    'End Sub
