...
Partial Class Pages_Roster_OT_Effective_Time
...
#Region "Panel Edit"
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        lblresult2.Text = ""
        If chkOPTION_DAY_TYPE.Checked = False Then
            ...
            End If
        Else
            If ValidateInsertAll() = True Then
                'mySQL.ExecuteSQL(ssql)
                RestructureSQLStringAndExecute(ssql, "", ddlOPTION_DAY_TYPE)
                pnlEdit.Visible = False
                pnlMain.Visible = True
                chkOPTION_DAY_TYPE.Checked = False
                BindGrid()
            End If
        End If
    End Sub
    Private Sub RestructureSQLStringAndExecute(ByVal ssql As String, ByVal strInput As String, ByVal ddl As DropDownList)
        Dim int As Integer = 0, strCompare As String = ""
        Dim htb As New Hashtable, de As New DictionaryEntry
        For int = 0 To ddl.Items.Count - 1
            If ddl.Items(int).Value <> "" Then
                strCompare = strCompare & "~" & ddl.Items(int).Text & "~,"
            End If
        Next
        ...
        If ssql.IndexOf(strCompare) <> -1 Then
            For int = 0 To ddl.Items.Count - 1
                If ddl.Items(int).Value <> "" Then
                    Dim tmpValue As String = ddl.Items(int).Value
                    htb.Add("ssql" & CStr(int), Left(ssql, ssql.IndexOf(strCompare)) & tmpValue & Right(ssql, Len(ssql) - Len(strCompare) - ssql.IndexOf(strCompare)))
                    tmpValue = ""
                End If
            Next
            mySQL.ExecuteSQLTransactionByHashtable(htb, "Insert Shift Profile", Session("Company").ToString, Session("EmpID").ToString)
        End If
    End Sub
...
#Region "Panel Action"
    ...
    Protected Sub imgBtnEdit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnEdit.Click
        If ActionValidateEdit() Then
            chkOPTION_DAY_TYPE.Visible = False
            pnlEdit.Visible = True
            pnldescription.Visible = True
            pnlMain.Visible = False
...
#Region "Sub & Function"
                            ...
                            Select Case UCase(Trim(myDS.Tables(0).Rows(i).Item(posType).ToString))
                                Case "0"
                                    If myLabel.Visible = True And myDropdownlist.Visible = True Then
                                        ...
                                        autonum = autonum + 1
                                        If myImageButton.ID = "imgbtnOPTION_DAY_TYPE" Then
                                            ...
                                            chkOPTION_DAY_TYPE.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                                            chkOPTION_DAY_TYPE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                                            chkOPTION_DAY_TYPE.Style.Add("position", "absolute")
                                        Else
                                            myImageButton.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                                            myImageButton.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                                            myImageButton.Style.Add("position", "absolute")
    ...
    End Function
    Private Function ValidateInsertAll() As Boolean
        lblresult2.Visible = True
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select Code,Option_Data_Type,Option_Editable,Option_Mandatory,Option_Default_Value,Name,Option_Primary_Key From Table_Field Where Table_Profile_Code='" & Form.ID & "' And Option_View_Card='YES' Order By Table_Profile_Code,Sequence_No")
        ssql = "Insert Into " & Form.ID & "("
            ...
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                Select Case myDS.Tables(0).Rows(i).Item(1).ToString
                    ...
                    Case "OPTION"
                        Dim myDropdownlist As DropDownList = Page.FindControl("ddl" & Trim(myDS.Tables(0).Rows(i).Item(0)))
                        If myDropdownlist.Visible = True Then
                            If myDropdownlist.Enabled = True Then
                                If mySetting.CheckTextNull(myDropdownlist.SelectedValue) = True Then
                                    If mySetting.ValidateInput(myDropdownlist, myDS.Tables(0).Rows(i).Item(1).ToString) = True Then
                                        ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & ","
                                        ssql2 = ssql2 & "'" & Trim(myDropdownlist.SelectedValue) & "',"
                                ...
                                End If
                            Else
                                If chkOPTION_DAY_TYPE.Checked = True Then
                                    ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & ","
                                    ssql2 = ssql2 & "'" & GetDDLSelectedText(myDropdownlist) & "',"
                                    ssql3 = ssql3 & myDS.Tables(0).Rows(i).Item(0) & " In (" & GetDDLSelectedValue(myDropdownlist) & ") And "
                                End If
                            End If
                        End If
                        myDropdownlist = Nothing
    ...
    End Function
    Private Function GetDDLSelectedValue(ByVal ddl As DropDownList) As String
        Dim strOutput As String = "", int As Integer
        For int = 0 To ddl.Items.Count - 1
            If ddl.Items(int).Value <> "" Then
                strOutput &= "'" & ddl.Items(int).Value & "',"
            End If
        Next
        ...
        Return strOutput
    End Function
    Private Function GetDDLSelectedText(ByVal ddl As DropDownList) As String
        Dim strOutput As String = "", int As Integer
        For int = 0 To ddl.Items.Count - 1
            If ddl.Items(int).Value <> "" Then
                strOutput &= "~" & ddl.Items(int).Text & "~,"
            End If
        Next
        If Right(strOutput, 1) = "," Then
            strOutput = Left(strOutput, Len(strOutput) - 1)
    ...
    Private Function ValidateUpdate() As Boolean
        ...
        Return True
    End Function
    Protected Sub chkOPTION_DAY_TYPE_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkOPTION_DAY_TYPE.CheckedChanged
        If chkOPTION_DAY_TYPE.Checked = True Then
            ddlOPTION_DAY_TYPE.Enabled = False
        Else
            ddlOPTION_DAY_TYPE.Enabled = True
        End If
    End Sub

    Protected Sub ddlOPTION_DAY_TYPE_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlOPTION_DAY_TYPE.SelectedIndexChanged
        If ddlOPTION_DAY_TYPE.SelectedValue = "" Then
            chkOPTION_DAY_TYPE.Enabled = True
        Else
            chkOPTION_DAY_TYPE.Enabled = False
        End If
    End Sub
#End Region
