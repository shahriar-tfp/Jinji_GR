...
Partial Class Pages_Roster_Shift_Roster
    ...
    Protected Sub calEventCalendar_DayRender(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DayRenderEventArgs) Handles calEventCalendar.DayRender
        Try
            ...
            If dtSelectedDate = e.Day.Date Then
                ...
                txtOCP_ID_SHIFT .Text = myLabel.Text
    ...
    Private Function ValidateInput() As Boolean
        ...
        If Trim(txtToDate.Text) <> "" Then
            ...
            End If
        End If
        If Trim(txtOCP_ID_SHIFT.Text.ToString) = "" Then
            lblResult.Visible = True
            lblResult.Text = "Please select a day type!"
            txtOCP_ID_SHIFT.Focus()
            Return False
        End If
    ...
    Private Sub PagePreload()
        Try
            ...
            mySetting.GetImgUrl(imgbtnOCP_ID_SHIFT, clsGlobalSetting.ImageType._LOOKUP, Session("Company").ToString)
            ...
            mySetting.GetLookupValue_ImageButton(imgbtnOCP_ID_TMS, Form.ID, txtOCP_ID_TMS.ID, "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID_TMS" & """," & """" & "" & """," & """" & Session("EmpID").ToString & """")
            mySetting.GetLookupValue_ImageButton(imgbtnOCP_ID_SHIFT, Form.ID, txtOCP_ID_SHIFT.ID, "Code", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID_SHIFT" & """," & """" & "" & """," & """" & Session("EmpID").ToString & """")
            ...
            ssql = "Select Code,Name From Table_Field Where Table_Profile_Code='" & Form.ID & _
                    "' And Code In ('OCP_ID_TMS','OCP_ID_SHIFT') Order By Sequence_No"
            myDS = New DataSet
            myDS = mySQL.ExecuteSQL(ssql)
    ...
    Private Sub BindCalendar()
        Try
            ...
            ssql = "Exec sp_tms_GetCalendarRecords '" & Session("Company").ToString & "','" & txtOCP_ID_TMS.Text.ToString.Trim & "','','','" & Session("SelectedYear").ToString & "','" & Session("SelectedMonth").ToString & "','" & Form.ID.ToString & "'"
            myDS = New DataSet
            myDS = mySQL.ExecuteSQL(ssql)
            If myDS.Tables.Count > 0 Then
                If myDS.Tables(0).Rows.Count > 0 Then
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSubmit.Click
        Try
            If ValidateInput() Then
                myDS = New DataSet
                ssql = "Exec sp_tms_InsUpdDelCalendar '" & Session("Company").ToString & "','" & Trim(txtFromDate.Text.ToString) & "','" & Trim(txtToDate.Text.ToString) & "','" & txtOCP_ID_TMS.Text.ToString & "','" & txtOCP_ID_SHIFT.Text.ToString.Trim & "','','','','" & Form.ID.ToString & "'"
