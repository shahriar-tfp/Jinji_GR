...
Partial Class PAGES_KPI_COMPANY_TARGET_DISTRIBUTIONS
...
#Region "Sub & Function"
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ...
                                Else
                                    ...
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        'Edit Default Value2'
                                        If myDST.Tables(0).Rows(0).Item(1).ToString.Trim = "" And myDST.Tables(0).Rows(0).Item(2).ToString.Trim = "" Then
                                            ssql2 = ssql2 & "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "'),''),"
                                        ElseIf myDST.Tables(0).Rows(0).Item(1).ToString.Trim <> "" And myDST.Tables(0).Rows(0).Item(2).ToString.Trim <> "" Then
                                            ssql2 = ssql2 & "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "',N'" & myDST.Tables(0).Rows(0).Item(1).ToString.Trim & "',N'" & Session("Company") & "'),''),"
                                        Else
                                        ...
                                        If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                            ...
                                            ElseIf myDST.Tables(0).Rows(0).Item(1).ToString.Trim <> "" And myDST.Tables(0).Rows(0).Item(2).ToString.Trim <> "" Then
                                                ssql3 = ssql3 & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "',N'" & myDST.Tables(0).Rows(0).Item(1).ToString.Trim & "',N'" & Session("Company") & "'),0) And "
                                            Else
                            ...
                            If myTextBox.Visible = True Then
                                If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                    ...
                                    Else
                                        ...
                                        If myDST2.Tables(0).Rows.Count > 0 Then
                                            'Edit Default Value2'
                                            If myDST2.Tables(0).Rows(0).Item(1).ToString.Trim = "" And myDST2.Tables(0).Rows(0).Item(2).ToString.Trim = "" Then
                                                ssql = ssql & myDS2.Tables(0).Rows(i).Item(0) & "=ISNULL(" & myDST2.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "'),'') And "
                                            ElseIf myDST2.Tables(0).Rows(0).Item(1).ToString.Trim <> "" And myDST2.Tables(0).Rows(0).Item(2).ToString.Trim <> "" Then
                                                ssql = ssql & myDS2.Tables(0).Rows(i).Item(0) & "=ISNULL(" & myDST2.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "',N'" & myDST2.Tables(0).Rows(0).Item(1).ToString.Trim & "',N'" & Session("Company") & "'),'') And "
                                            Else
    ...
    Private Sub SetLabelValueDisplayText()
        If ddlGROUP_BY.SelectedValue <> "" Then
            lblOCP_ID.Text = ddlGROUP_BY.SelectedItem.Text.ToString
        Else
...
#End Region
    ...
    End Sub
    Protected Sub ddlGROUP_BY_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlGROUP_BY.SelectedIndexChanged
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID" & """," & "" & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlGROUP_BY.SelectedValue & """"
        imgbtnOCP_ID.Attributes.Remove("onclick")

        mySetting.GetLookupValue_ImageButton(imgbtnOCP_ID, Form.ID, "txtOCP_ID", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID" & """," & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlGROUP_BY.SelectedValue & """")
        SetLabelValueDisplayText()
    End Sub

    Protected Sub txtTARGET_REVENUE_PERCENTAGE_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtTARGET_REVENUE_PERCENTAGE.TextChanged
        ssql = "select Convert(int,Target_Revenue *" & txtTARGET_REVENUE_PERCENTAGE.Text & " /100) from Company_revenue where Revenue_descriptions =N'" & ddlOPTION_REVENUE_DESCRIPTION.SelectedValue & "'"
        myDS = mySQL.ExecuteSQL(ssql)

        If Not myDS Is Nothing Then
            If myDS.Tables.Count = 1 Then
                If myDS.Tables(0).Rows.Count > 0 Then
                    txtTARGET_REVENUE.Text = myDS.Tables(0).Rows(0).Item(0).ToString
                Else
                    txtTARGET_REVENUE.Text = ""
                End If
            Else
                txtTARGET_REVENUE.Text = ""
            End If
