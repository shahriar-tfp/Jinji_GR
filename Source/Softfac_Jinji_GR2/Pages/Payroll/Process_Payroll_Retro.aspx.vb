...
Partial Class Pages_Payroll_Process_Payroll_Retro
    ...
    Private Sub PagePreload()
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            ...
            lblTitle.Text = " Process Payroll Retro"
    ...
    Private Sub BindListBox(ByVal lstBox As ListBox)
        mySetting.BindListBox(lstBox, "Exec sp_pr_ProcessPayroll_Retro '" & Session("Company").ToString & "','" & Session("EmpID").ToString & "','" & Session("Module").ToString & "','" & ddlFILTER_BY.SelectedItem.Value.ToString & "','','FILTER'", 0, 2)
    End Sub
        ...
        If ValidateProcess() = True Then
                ...
                For Me.i = 0 To myGridView.Rows.Count - 1
                    ...
                    If chkBox.Checked = True Then
                        htb.Add("ssql" & CStr(i).ToString, "Insert Into Employee_Payroll_Process_Temp Values(N'" & Session("Company").ToString & "'," & _
                                                "dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(i).Cells(1).Text.ToString.Replace("&amp;", "&").Replace("'", "''") & "'),N'" & txtYEAR.Text.ToString & "',N'" & _
            ...
            Else
                ssql = "Exec sp_pr_InsertEmployeeForPayrollProcess N'" & Session("Company").ToString & "',N'" & Session("Module").ToString & "',N'" & Session("EmpID").ToString & _
                    ...
                    "',N'" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim.Replace("'", "''") & "',N'" & Session("SPID").ToString & "'"
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            End If

            ssql = "Exec sp_pr_calPayrollProcessing_Retro N'" & Session("Company").ToString & "',N'" & _
