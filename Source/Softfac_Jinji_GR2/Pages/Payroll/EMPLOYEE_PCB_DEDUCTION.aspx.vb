...
Partial Class PAGES_PAYROLL_EMPLOYEE_PCB_DEDUCTION
...
#Region "Panel Edit"
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        If ValidateInsert() = True Then
            If ValidateMaxValue() Then
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                pnlEdit.Visible = False
                pnlMain.Visible = True
                BindGrid()
            Else
                ShowMessage(txtPCB_DEDUCTION_PROFILE_CODE.Text & " exists Maximum Amount(" & MaxAmount & ")")
            End If
        End If
    End Sub
...
#Region "Sub & Function"
    ...
    Private Function ValidateMaxValue() As Boolean
        ValidateMaxValue = False
        'Dim ssqlChk As String, ssqlGetID As String
        'Dim myDS1 As DataSet, mydsGetID As DataSet

        'ssqlGetID = "Select dbo.fn_ReturnCompanyProfileCode('" & txtCOMPANY_PROFILE_CODE.Text & _
        '"'), dbo.fn_ReturnEmpIDByCodeName(dbo.fn_ReturnCompanyProfileCode('" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & _
        'txtEMPLOYEE_PROFILE_ID.Text & "'),dbo.fn_GetCode_PCB_DEDUCTION_PROFILE_Vw('" & txtPCB_DEDUCTION_PROFILE_CODE.Text & "')"
        'mydsGetID = New DataSet
        'mydsGetID = mySQL.ExecuteSQL(ssqlGetID)
        'If mydsGetID.Tables(0).Rows.Count > 0 Then
        '    ssqlChk = "EXEC sp_pr_chkEmployeePCBDeduction '" & mydsGetID.Tables(0).Rows(0).Item(0).ToString & "','" _
        '    & mydsGetID.Tables(0).Rows(0).Item(1).ToString & "','" & txtMONTH.Text & "','" & txtYEAR.Text & "','" _
        '    & mydsGetID.Tables(0).Rows(0).Item(2).ToString & "','" & txtAMOUNT.Text & "'"
        '    myDS1 = New DataSet
        '    myDS1 = mySQL.ExecuteSQL(ssqlChk)

        '    If myDS1.Tables(0).Rows(0).Item(0).ToString = "1" Then
        '        MaxAmount = myDS1.Tables(0).Rows(0).Item(1).ToString
        '        Exit Function
        '    End If
        'End If
