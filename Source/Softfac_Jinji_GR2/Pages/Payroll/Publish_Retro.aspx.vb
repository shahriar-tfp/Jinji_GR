...
Partial Class Pages_Payroll_Publish_Retro
    ...
    Private Sub PagePreload()
        ...
        lblFILTER_BY.Text = "Option Status"
        lblFILTER_BY.CssClass = "wordstyle3"
        lblPayMonth.Text = "Payment Year / Month"
        lblPayMonth.CssClass = "wordstyle3"
        lblPayCycle.Text = "Payment Cycle"
        lblPayCycle.CssClass = "wordstyle3"
        ...
        SetInvisibleAll()
        ssql = "Exec sp_app_GetInfo 'Publish_Retro','" & Session("Company") & "','" & Session("Module") & "'"
        ...
        Do Until counter = sysYear + 1
            ...
            ddlPayYear.Items.Add(counter)
            counter = counter + 1
        Loop
        ddlYEAR.SelectedValue = sysYear
        mySetting.GetDropdownlistValue("EMPLOYEE_OT_ADJUSTMENT", "MONTH", ddlMONTH)
        ddlMONTH.SelectedIndex = DatePart(DateInterval.Month, Now())

        ddlPayYear.SelectedValue = sysYear
        mySetting.GetDropdownlistValue("EMPLOYEE_OT_ADJUSTMENT", "MONTH", ddlPayMonth)
        ddlPayMonth.SelectedIndex = DatePart(DateInterval.Month, Now())
        ...
        mySetting.GetBtnImgUrl(imgbtnUpdate3, Session("Company").ToString, btnColourDef, "btnUpdate.png")
        ...
        mySetting.GetImgUrl(imgKeyPayMonth, clsGlobalSetting.ImageType._KEY, Session("Company").ToString)
        mySetting.GetImgUrl(imgKeyPayCycle, clsGlobalSetting.ImageType._KEY, Session("Company").ToString)
        ...
        End If
        mySetting.GetDropdownlistValue("EMPLOYEE_PAYROLL_RETRO_BALANCE", "PAY_OPTION_PAY_CYCLE", ddlPayCycle)
        If ddlPayCycle.Items.Count >= 2 Then
            ddlPayCycle.SelectedIndex = 0
        End If
        mySetting.GetDropdownlistValue("EMPLOYEE_PAYROLL_RETRO_BALANCE", "TRANSFER_STATUS", ddlFILTER_BY)
        ...
        'get Page Title
        ssql = "exec sp_sa_GetPageTitle 'PUBLISH_EPAYSLIP'"
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If myDS.Tables(0).Rows.Count > 0 Then
    ...
    Private Sub BindGrid()
        Try
            ...
            End If
            If SearchCriteria = False Then
                ssql = "Exec sp_pr_selPublishRetro N'" & Session("Company").ToString & "',N'" & ddlYEAR.SelectedValue & "',N'" & ddlMONTH.SelectedValue & "',N'" & ddlFILTER_BY.SelectedValue & "'"
                'Else
    ...
    Protected Sub ddlFILTER_BY_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlFILTER_BY.SelectedIndexChanged
        ...
        ElseIf ddlFILTER_BY.SelectedValue.ToString.ToUpper = "" Then
            ...
            chkSelectAll.Visible = False
            chkClearAll.Visible = False
        ElseIf ddlFILTER_BY.SelectedValue.ToUpper = "PAYROLL" Then
            imgbtnUpdate.Visible = False
            imgbtnUpdate2.Visible = False
            imgbtnUpdate3.Visible = True
            SetInvisibleEmployeeProfileID()
            pnlProgress.Visible = False
            imgBtnSearch_Click(Nothing, Nothing)
        ElseIf ddlFILTER_BY.SelectedValue.ToUpper = "RETRO" Then
            imgbtnUpdate.Visible = True
            imgbtnUpdate2.Visible = False
            imgbtnUpdate3.Visible = False
            SetInvisibleEmployeeProfileID()
            pnlProgress.Visible = False
            imgBtnSearch_Click(Nothing, Nothing)
        Else
            ...
            If pnlGridview.Visible = True Then
                For Me.i = 0 To myGridView.Rows.Count - 1
                    ...
                    If chkBox.Checked = True Then
                        htb.Add("ssql" & CStr(i).ToString, "Insert Into Employee_Payroll_Process_Temp Values(N'" & Session("Company").ToString & "'," & _
                                                "dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(i).Cells(1).Text.ToString.Replace("&amp;", "&").Replace("'", "''") & "'),N'" & ddlYEAR.Text.ToString & "',N'" & _
                                                ddlMONTH.Text.ToString & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & Session("SPID").ToString & "')")
                    End If
                Next
                mySQL.ExecuteSQLTransactionByHashtable(htb, "Payroll Process Data", Session("Company").ToString, Session("EmpID").ToString)
                htb = Nothing
            Else
                ssql = "Exec sp_pr_InsertEmployeeForPayrollProcess N'" & Session("Company").ToString & "',N'" & Session("Module").ToString & "',N'" & Session("EmpID").ToString & _
                    "',N'" & ddlYEAR.Text.ToString & "',N'" & ddlMONTH.Text.ToString & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & ddlFILTER_BY.SelectedValue.ToString & _
                    "',N'" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "',N'" & Session("SPID").ToString & "'"
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            End If

            ssql = "Exec sp_pr_calPayrollProcessing N'" & Session("Company").ToString & "',N'" & _
                            ddlPAY_CYCLE.SelectedValue & "',N'" & Trim(ddlMONTH.Text.ToString) & "',N'" & _
                            Trim(ddlYEAR.Text.ToString) & "',N'" & ddlFILTER_BY.SelectedValue & "',N'" & Session("SPID").ToString & "'"
            myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            If Not myDS Is Nothing Then
                lblSysName.Visible = True
                lblSysName.Text = myDS.Tables(0).Rows(0).Item(0).ToString
                Dim myDT As New DataTable, myDC As DataColumn
                myDT = myDS.Tables(0)
                myDC = myDS.Tables(0).Columns(0)
                myDT.Columns.Remove(myDC)
                gvMessage.Visible = True
                gvMessage.DataSource = myDT.DefaultView
                gvMessage.DataBind()
            End If
    ...
    Protected Sub ddlMONTH_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlMONTH.SelectedIndexChanged
        If ddlFILTER_BY.SelectedValue.ToUpper = "PAYROLL" Then
            imgbtnUpdate.Visible = False
            imgbtnUpdate2.Visible = False
            imgbtnUpdate3.Visible = True
            SetInvisibleEmployeeProfileID()
    ...
    Protected Sub imgbtnUpdate_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnUpdate.Click
        ...
        If pnlGridview.Visible = True Then
            For Me.i = 0 To myGridView.Rows.Count - 1
                Dim chkBox As CheckBox = myGridView.Rows(i).Cells(0).Controls(1)
                If chkBox.Checked = True Then
                    ssql1 = "select isnull(dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(i).Cells(1).Text.ToString.Replace("&amp;", "&").Replace("'", "''") & "'),''),isnull(dbo.fn_getocpidbycodename(N'" & myGridView.Rows(i).Cells(5).Text.ToString.Replace("&amp;", "&") & "','SALARY'),'')"
                    myDS = mySQL.ExecuteSQL(ssql1)
                    If myDS.Tables(0).Rows(0).Item(0).ToString <> "" Then
                        ssql = "exec [sp_pr_UpdPublishRetro] '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & mySetting.UnDisplayDateTime(myGridView.Rows(i).Cells(11).Text.ToString.Replace("&amp;", "&"), Session("Company"), Session("Module")) & "','" & ddlMONTH.SelectedValue & "','" _
                        & ddlYEAR.SelectedValue & "','" & ddlFILTER_BY.SelectedValue & "','" & ddlPayMonth.SelectedValue & "','" & ddlPayYear.SelectedValue & "','" & _
                        ddlPayCycle.SelectedValue & "','PAYROLL','ADD'"
                        mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                    End If
                    ssql1 = Nothing
                    myDS = Nothing
                End If
            Next
            ssql1 = "exec sp_pr_UpdPublishRetro '','','','','','','','','','','Publish Result'"
    ...
    Protected Sub imgbtnUpdate2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnUpdate2.Click
        ...
        If pnlGridview.Visible = True Then
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                End If
            Next
            ssql1 = "exec sp_pr_UpdPublishRetro '','','','','','','','','','','RePublish Result'"
            myDS = mySQL.ExecuteSQL(ssql1, Session("Company").ToString, Session("EmpID").ToString)
            lblSysName.Visible = True
            lblSysName.Text = myDS.Tables(0).Rows(0).Item(0).ToString
            pnlProgress.Visible = True
    ...
    Protected Sub imgbtnUpdate3_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnUpdate3.Click
        Dim SPID As String = mySQL.GetSPID
        Dim ssql1 As String, myDS As DataSet
            ...
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                If chkBox.Checked = True Then
                    ...
                    If myDS.Tables(0).Rows(0).Item(0).ToString <> "" Then
                        ssql = "exec [sp_pr_UpdPublishRetro] '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & mySetting.UnDisplayDateTime(myGridView.Rows(i).Cells(11).Text.ToString.Replace("&amp;", "&"), Session("Company"), Session("Module")) & "','" & ddlMONTH.SelectedValue & "','" _
                        & ddlYEAR.SelectedValue & "','" & ddlFILTER_BY.SelectedValue & "','" & ddlPayMonth.SelectedValue & "','" & ddlPayYear.SelectedValue & "','" & _
                        ddlPayCycle.SelectedValue & "','RETRO','DEL'"
                        mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                    End If
                    ssql1 = Nothing
                    myDS = Nothing
                End If
            Next
            ssql1 = "exec sp_pr_UpdPublishRetro '','','','','','','','','','','UnPublish Result'"
            myDS = mySQL.ExecuteSQL(ssql1, Session("Company").ToString, Session("EmpID").ToString)
            lblSysName.Visible = True
            lblSysName.Text = myDS.Tables(0).Rows(0).Item(0).ToString
            pnlProgress.Visible = True
