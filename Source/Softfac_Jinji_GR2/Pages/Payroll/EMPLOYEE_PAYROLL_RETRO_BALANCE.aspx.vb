...
Partial Class PAGES_PAYROLL_EMPLOYEE_PAYROLL_RETRO_BALANCE
...
#Region "Panel Action"
    ...
    Protected Sub imgBtnEdit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnEdit.Click
        If ActionValidateEdit() = True And ActionValidatePaid() = True Then
            pnlEdit.Visible = True
            pnldescription.Visible = True
            pnlMain.Visible = False
    ...
    Protected Sub imgBtnDelete_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnDelete.Click
        If ActionValidateDelete() = True And ActionValidatePaid() = True Then
            For Me.i = 0 To myGridView.Rows.Count - 1
...
#Region "Sub & Function"
    ...
    Private Function ActionValidatePaid() As Boolean
        RecFound = True
        For Me.i = 0 To myGridView.Rows.Count - 1
            Dim chkDelete As CheckBox = myGridView.Rows(i).Cells(0).Controls(1)
            If chkDelete.Checked Then
                ssql = "Select TRANSFER_STATUS from employee_payroll_retro_balance " & _
                "where EMPLOYEE_PROFILE_ID = dbo.fn_ReturnEmpIDByCodeName(N'" & Session("company").ToString & "',N'" & myGridView.Rows(i).Cells(1).Text & "') and " & _
                "Year = N'" & myGridView.Rows(i).Cells(2).Text & "' and Month = N'" & myGridView.Rows(i).Cells(3).Text & "' and " & _
                "OPTION_PAY_CYCLE = dbo.fn_GetOptionCode('" & Form.ID & "','OPTION_PAY_CYCLE',N'" & myGridView.Rows(i).Cells(4).Text & "') and " & _
                "OCP_ID_SALARY = dbo.fn_GetOCPIDByCodeName(N'" & myGridView.Rows(i).Cells(6).Text & "','SALARY') and " & _
                "PROCESS_DATE = dbo.fn_UnDisplayDateTime(N'" & myGridView.Rows(i).Cells(12).Text & "',N'" & Session("Company").ToString & "',N'" & Session("Module").ToString & "')"

                Dim myDST As New DataSet
                myDST = mySQL.ExecuteSQL(ssql)
                If myDST.Tables(0).Rows.Count > 0 Then
                    If myDST.Tables(0).Rows(0).Item(0).ToString = "PAID" Then
                        RecFound = False
                    End If
                End If
            End If
        ...
        If RecFound = True Then
            Return True
        Else
            ShowMessage("Transfer Status = PAID can not Edit and Delete. Please select the Transfer Status Not Equals to PAID!")
            Return False
        End If
    End Function
