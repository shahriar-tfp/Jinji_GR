...
Partial Class PAGES_PAYROLL_ALLOWANCE_IMPORT
    ...
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                ...
                Dim myData As New OleDbDataAdapter("SELECT * FROM [Allowance$]", strConn)
        ...
        Try
            If GVUpload.Rows.Count > 0 Then
                For Me.i = 0 To GVUpload.Rows.Count - 1
                    ssql = "insert into Employee_Benefit select ISNULL(dbo.fn_GetEmpID('" & GVUpload.Rows(i).Cells(1).Text.Trim.ToUpper & "'),''),"
                    ...
                    ssql = ssql & "DBO.FN_GETOPTIONCODE(N'EMPLOYEE_BENEFIT',N'OPTION_PAY_TYPE',N'" & GVUpload.Rows(i).Cells(6).Text.Trim.ToUpper & "'),"
                    ssql = ssql & "DBO.FN_GETOPTIONCODE(N'EMPLOYEE_BENEFIT',N'OPTION_Calculate_On',N'" & GVUpload.Rows(i).Cells(7).Text.Trim.ToUpper & "'),"
                    ssql = ssql & "DBO.FN_GETOPTIONCODE(N'EMPLOYEE_BENEFIT',N'OPTION_PAY_Cycle',N'" & GVUpload.Rows(i).Cells(8).Text.Trim.ToUpper & "')"
                    mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                    ssql = Nothing
                Next
            End If
