...
Partial Class PAGES_PAYROLL_BONUS_IMPORT
...
#End Region
    ...
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                Dim postedFile = txtFileName.PostedFile
                ...
                Dim myData As New OleDbDataAdapter("SELECT * FROM [EmpBonus$]", strConn)
        ...
        Try
            If GVUpload.Rows.Count > 0 Then
                For Me.i = 0 To GVUpload.Rows.Count - 1
                    ssql2 = "select ISNULL(dbo.fn_GetEmpID('" & GVUpload.Rows(i).Cells(1).Text.Trim.ToUpper & "'),''),"
                    ssql2 = ssql2 & "ISNULL(dbo.fn_GetOCPIDByCodeName('" & GVUpload.Rows(i).Cells(2).Text.Trim.ToUpper & "','SALARY'),''),"
                    ssql2 = ssql2 & "ISNULL(dbo.fn_GetOCPIDByCodeName('" & GVUpload.Rows(i).Cells(8).Text.Trim.ToUpper & "','APPRAISAL'),'')"
                    myDS = mySQL.ExecuteSQL(ssql2, Session("Company").ToString, Session("EmpID").ToString)
                    If myDS.Tables.Count > 0 Then

                        ssql = "exec sp_pr_importBonus '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','"
                        ssql = ssql & myDS.Tables(0).Rows(0).Item(1).ToString & "',"
                        ssql = ssql & GVUpload.Rows(i).Cells(3).Text.Trim.ToUpper & ",'"
                        ssql = ssql & GVUpload.Rows(i).Cells(4).Text.Trim.ToUpper & "',"
                        ssql = ssql & "'" & mySetting.ConvertDateToDecimal(Trim(GVUpload.Rows(i).Cells(5).Text.Trim.ToUpper), Session("Company").ToString, Session("Module").ToString) & "',"
                        ssql = ssql & "'" & mySetting.ConvertDateToDecimal(Trim(GVUpload.Rows(i).Cells(6).Text.Trim.ToUpper), Session("Company").ToString, Session("Module").ToString) & "','"
                        ssql = ssql & GVUpload.Rows(i).Cells(7).Text.Trim.ToUpper & "','"
                        ssql = ssql & myDS.Tables(0).Rows(0).Item(2).ToString & "','"
                        ssql = ssql & Session("EmpID").ToString & "'"
                        myDS2 = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)

                        If myDS2.Tables.Count > 0 Then
                            GVUpload.Rows(i).Cells(0).Text = myDS2.Tables(0).Rows(0).Item(0).ToString
                        End If
                    End If
                    ssql = Nothing
                Next
