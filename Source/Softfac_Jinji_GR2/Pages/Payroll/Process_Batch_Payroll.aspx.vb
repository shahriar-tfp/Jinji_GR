...
Partial Class Pages_Payroll_Process_Batch_Payroll
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
    End Sub
    Private Sub ShowBatchState()
        Dim myDS = New DataSet
        myDS = mySQL.ExecuteSQL("select convert(nvarchar,querydate) as Date,state as State,Message,ProcessTime,Active, Resign, NewJoin, Skip from dbo.Payroll_Batch_Process", Session("Company").ToString, Session("EmpID").ToString)
        If Not myDS Is Nothing Then
            grdBatchProcess.DataSource = myDS.Tables(0).DefaultView
            grdBatchProcess.DataBind()
        End If
    End Sub
    Private Sub PagePreload()
        ...
        txtMONTH.Text = DatePart(DateInterval.Month, Now())
        ...
        '--------------Daniel Song 2008.6.26-------------
        lblDateStart.Text = "Batch Start Date"
        ...
        Dim guessBatchTime As DateTime = DateTime.Now.AddMinutes(5)

        txtDateStart.Text = guessBatchTime.ToString("dd/MM/yyyy")

        mySetting.GetImgUrl(imgKeyHOUR_MINUTE, clsGlobalSetting.ImageType._KEY, Session("Company").ToString)
        lblHOUR.Text = "Batch Hour: "
        lblHOUR.CssClass = "wordstyle10"
        Dim h As Integer = DatePart(DateInterval.Hour, guessBatchTime)
        If h > 12 Then
            txtHOUR.Text = Convert.ToString(h - 12)
            lstAP.Items.FindByValue("AM").Selected = False
            lstAP.Items.FindByValue("PM").Selected = True
        Else
            txtHOUR.Text = h.ToString()
        End If

        lblMINUTE.Text = "Batch Minute : "
        lblMINUTE.CssClass = "wordstyle10"
        txtMINUTE.Text = DatePart(DateInterval.Minute, guessBatchTime)
    ...
    Private Sub BindGrid()
        Try
            ...
            End If
                ...
                End If
                If SearchCriteria = False Then
                    ssql = "Exec sp_sa_GetTableRecordsWithSecurity N'" & Session("Company").ToString & "',N'" & Session("EmpID").ToString & "',N'" & Session("Module").ToString & "',N'" & Form.ID & "','99999','1'"
                Else
    ...
    Protected Sub imgbtnPROCESS_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnPROCESS.Click
        If ValidateProcess() = True Then
            ...
            If pnlGridview.Visible = True Then
                For Me.i = 0 To myGridView.Rows.Count - 1
                    ...
                    If chkBox.Checked = True Then
                        ...
                        '                       "dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(i).Cells(1).Text.ToString.Replace("&amp;", "&") & "'),N'" & txtYEAR.Text.ToString & "',N'" & _
                        '                      txtMONTH.Text.ToString & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & Session("SPID").ToString & "')")
                        If query.ToString().Equals("") Then

                        Else
                            query.Append(" ^ ")
                        End If
                        query.Append(" " & "Insert Into Employee_Payroll_Process_Temp Values(N'" & Session("Company").ToString & "'," & _
                                                "dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(i).Cells(1).Text.ToString.Replace("&amp;", "&") & "'),N'" & txtYEAR.Text.ToString & "',N'" & _
                                                txtMONTH.Text.ToString & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & Session("SPID").ToString & "')")
                        num += 1
                    End If
            ...
            Else
                ...
                '"',N'" & txtYEAR.Text.ToString & "',N'" & txtMONTH.Text.ToString & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & ddlFILTER_BY.SelectedValue.ToString & _
                '"',N'" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "',N'" & Session("SPID").ToString & "'"
                query.Append("Exec sp_pr_InsertEmployeeForPayrollProcess N'" & Session("Company").ToString & "',N'" & Session("Module").ToString & "',N'" & Session("EmpID").ToString & _
                    "',N'" & txtYEAR.Text.ToString & "',N'" & txtMONTH.Text.ToString & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & ddlFILTER_BY.SelectedValue.ToString & _
                    "',N'" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "',N'" & Session("SPID").ToString & "'")
                num += 1
            ...
            'ddlPAY_CYCLE.SelectedValue & "',N'" & Trim(txtMONTH.Text.ToString) & "',N'" & _
            'Trim(txtYEAR.Text.ToString) & "',N'" & ddlFILTER_BY.SelectedValue & "',N'" & Session("SPID").ToString & "'"
            If query.ToString().Equals("") Then

            Else
                query.Append(" ^ ")
            End If
            query.Append("Exec sp_pr_calPayrollProcessing N'" & Session("Company").ToString & "',N'" & _
                            ddlPAY_CYCLE.SelectedValue & "',N'" & Trim(txtMONTH.Text.ToString) & "',N'" & _
                            Trim(txtYEAR.Text.ToString) & "',N'" & ddlFILTER_BY.SelectedValue & "',N'" & Session("SPID").ToString & "'")
            num += 1
            'myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            ...
            '    gvMessage.DataBind()
            'End If
            Dim datestart = txtDateStart.Text.ToString()
            Dim dd = Mid(datestart, 1, 2)
            Dim mm = Mid(datestart, 4, 2)
            Dim yy = Mid(datestart, 7, 4)
            datestart = mm + "/" + dd + "/" + yy
            Dim batchdate As String = datestart + " " + txtHOUR.Text.ToString() + ":" + txtMINUTE.Text.ToString() + " " + lstAP.SelectedItem.Value.ToString()

            Dim result As String = mySQL.AddBatchQuery(query, Session("EmpID").ToString(), num, batchdate)

            If result.Equals("true") Then
                'Dim scriptString As String = "<script language=JavaScript>"
                'scriptString += "alert('test message');"
                'scriptString += "</script>"
