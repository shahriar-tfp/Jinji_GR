...
Partial Class PAGES_PAYROLL_IMPORT_EMPLOYEE_PAYROLL_SUMMARY
    ...
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                ...
                Dim myData As New OleDbDataAdapter("SELECT * FROM [sheet2$]", strConn)
                myData.TableMappings.Add("Table", "ExcelTest")
                myData.Fill(myDS)

                Dim abs As Integer = myDS.Tables(0).DefaultView.Table.Columns.Count
                Do Until abs = 23
                    myDS.Tables(0).Columns.RemoveAt(abs - 1)
                    abs = abs - 1
                Loop
    ...
    Protected Sub imgBtnUpdateImport_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdateImport.Click
        Try
            ...
            If GVUpload.Rows.Count > 0 Then
                Dim a As Integer = GVUpload.Rows.Count
                ssql = "delete from IMPORT_EMPLOYEE_PAYROLL_SUMMARY"
                mySQL.ExecuteSQL(ssql)
                ssql = Nothing
                For Me.i = 0 To GVUpload.Rows.Count - 1
                    ssql1 = "select company_profile_code,ID from employee_profile where code = N'" & GVUpload.Rows(i).Cells(6).Text.Trim.ToUpper & "'"
                    ...
                    If myds1.Tables(0).Rows.Count > 0 Then
                        If CInt(myds1.Tables(0).Rows(0).Item(1)) > 0 Then
                            company = myds1.Tables(0).Rows(0).Item(0).ToString
                            employee_id = myds1.Tables(0).Rows(0).Item(1).ToString
                            If CInt(employee_id) > 0 Then
                                ssql = "insert into IMPORT_EMPLOYEE_PAYROLL_SUMMARY values(N'"
                                ssql = ssql + company + "',N'"
                                ssql = ssql + employee_id + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(6).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(5).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(1).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(2).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(3).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(4).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(7).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(8).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(9).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(10).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(11).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(12).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(13).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(14).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(15).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(16).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(17).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(18).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(19).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(20).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(21).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(22).Text.Trim + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(23).Text.Trim + "',"
                                ssql = ssql & "N'" & Session("EmpID").ToString & "',dbo.fn_getcurrentdatetime(getdate()),N'" & Session("EmpID").ToString & "',dbo.fn_getcurrentdatetime(getdate()),'')"
                                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                                ssql = Nothing
                                GVUpload.Rows(i).Cells(0).Text = "Sucessfully Insert."
                            Else
                                GVUpload.Rows(i).Cells(0).Text = "Cannot Found Employee Code."
                                ssql = "insert into IMPORT_EMPLOYEE_PAYROLL_SUMMARY values(N'"
                                ssql = ssql + company + "',N'"
                                ssql = ssql + employee_id + "',N'"
                                ssql = ssql + GVUpload.Rows(i).Cells(6).Text.Trim + "',N'"
                    ...
                    myds1 = Nothing
                Next
                ssql = "exec sp_pr_importPayrollSummary"
                myDS2 = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                If myDS2.Tables.Count > 1 Then
                    GVUpload.DataSource = myDS2.Tables(1)
                    GVUpload.DataBind()
                    For Me.i = 0 To myDS2.Tables(0).Rows.Count - 1
                        GVUpload.Rows(i).Cells(0).Text = myDS2.Tables(0).Rows(i).Item(0).ToString
                    Next
                End If
