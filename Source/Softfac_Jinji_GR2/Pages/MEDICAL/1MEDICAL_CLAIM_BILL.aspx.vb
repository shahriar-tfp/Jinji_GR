...
Partial Class Pages_MEDICAL_MEDICAL_CLAIM_BILL
...
#End Region
    ...
    Private Sub PagePreload()
        ...
        ddlTYPE.SelectedValue = "CLAIM"
        ddlSTATUS.Enabled = False
        ddlSTATUS.SelectedValue = "PENDING"
        txtAUTOAPPROVALNO.Enabled = False
    ...
    '            "','" & myDS.Tables(0).Rows(0).Item(3).ToString & "','" & txtBENEFICIARYVISITER.Text & "','" & _
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        ...
        ElseIf myds.Tables(0).Rows(0).Item(2).ToString = "" Then
            ...
            txtCLAIMTYPE.Focus()
            Return False
        ElseIf IsNumeric(mySetting.ConvertDateToDecimal(Trim(txtBILLDATE.Text), Session("Company").ToString, Session("Module").ToString)) = False Then
            lblresult2.Text = "[" & lblBILLDATE.Text & "] Is A Required Field! Invalid Input DateTime Format!"
            txtBILLDATE.Focus()
            Return False
            'ElseIf IsNumeric(mySetting.ConvertDateToDecimal(Trim(txtPAYMENTMONTH.Text), Session("Company").ToString, Session("Module").ToString)) = False Then
            '    lblresult2.Text = "[" & lblPAYMENTMONTH.Text & "] Is A Required Field! Invalid Input DateTime Format!"
        ...
        ElseIf IsNumeric(txtCLAIMAMT.Text) = False Then
            ...
            txtCLAIMAMT.Focus()
            Return False
        ElseIf IsNumeric(txtBILLAMT.Text) = False Then
            lblresult2.Text = "[" & lblBILLAMT.Text & "] Is Invalid Input Numeric Format!"
            txtBILLAMT.Focus()
            Return False
        Else
            Return True
    ...
    Private Sub DisableField()
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("SELECT Code,Option_Primary_Key,Option_Data_Type,Option_Mandatory FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' AND Option_View_Card='YES' and Code not in ('CLAIMAMT','BILLNO','BILLDATE','BILLAMT','PAYMENTMONTH','AUTOAPPROVALNO','STATUS') ORDER BY SEQUENCE_NO")
        For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
            Dim myImageKey As ImageButton = Page.FindControl("imgKey" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
            Dim myImageButton As ImageButton = Page.FindControl("imgbtn" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
            Dim myLabel As Label = Page.FindControl("lbl" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
    ...
    Protected Sub txtEMPLOYEE_PROFILE_ID_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtEMPLOYEE_PROFILE_ID.TextChanged
        Dim empid As String
        myDS = mySetting.GetEmployeeIDbyCodeName(Session("Company").ToString, txtEMPLOYEE_PROFILE_ID.Text)
        If myDS.Tables(0).Rows.Count > 0 Then
            empid = myDS.Tables(0).Rows(0).Item(0).ToString
        Else
            empid = ""
        End If
        mySetting.GetLookupValue_ImageButton(imgbtnCLAIMTYPE, Form.ID, "txtCLAIMTYPE", "CodeName", "Exec sp_md_GetMedicalEntitlementCodeName " & """" & Session("Company").ToString & """, """ & empid & """", txtEMPLOYEE_PROFILE_ID.Text)
    ...
    Protected Sub txtAUTOAPPROVALNO_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtAUTOAPPROVALNO.TextChanged
        ssql = "Exec sp_md_SelMedicalChit 'CLAIM','" & txtAUTOAPPROVALNO.Text & "'"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)

        If myDS.Tables(0).Rows.Count > 0 Then
            txtCOMPANY_PROFILE_CODE.Text = myDS.Tables(0).Rows(0).Item(0).ToString
            txtEMPLOYEE_PROFILE_ID.Text = myDS.Tables(0).Rows(0).Item(1).ToString
            txtCLAIMTYPE.Text = myDS.Tables(0).Rows(0).Item(2).ToString
            txtPANELDOCTOR_PROFILE.Text = myDS.Tables(0).Rows(0).Item(3).ToString
            ddlBENEFICIARYVISITER.SelectedValue = myDS.Tables(0).Rows(0).Item(4).ToString
            txtVISITDATE.Text = myDS.Tables(0).Rows(0).Item(5).ToString
            txtREASON.Text = myDS.Tables(0).Rows(0).Item(6).ToString
            txtENTITLEMENTAMT.Text = myDS.Tables(0).Rows(0).Item(7).ToString
            txtUPTODATE_BALANCE.Text = myDS.Tables(0).Rows(0).Item(8).ToString
            txtCLAIMAMT.Text = myDS.Tables(0).Rows(0).Item(9).ToString
            ddlTYPE.SelectedValue = myDS.Tables(0).Rows(0).Item(10).ToString
            ddlSTATUS.SelectedValue = myDS.Tables(0).Rows(0).Item(11).ToString
            txtBILLNO.Text = myDS.Tables(0).Rows(0).Item(12).ToString
            txtBILLDATE.Text = myDS.Tables(0).Rows(0).Item(13).ToString
            txtBILLAMT.Text = myDS.Tables(0).Rows(0).Item(14).ToString
            txtPAYMENTMONTH.Text = myDS.Tables(0).Rows(0).Item(15).ToString
        End If
    End Sub
    ...
    Protected Sub imgBtnUpdate_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdate.Click
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "')," & _
        "dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & txtEMPLOYEE_PROFILE_ID.Text & "'), " & _
        "dbo.fn_GetCode_MEDICAL_CLAIMTYPE_Vw(N'" & txtCLAIMTYPE.Text & "')," & _
        "dbo.fn_GetCode_MEDICAL_PANELDOCTOR_Vw('" & txtPANELDOCTOR_PROFILE.Text & "')"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)

        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                If txtAUTOAPPROVALNO.Text = "" Then
                    ssql1 = "exec sp_md_InsUpdDelMedicalTransaction '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & _
                            myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & _
                            myDS.Tables(0).Rows(0).Item(3).ToString & "','" & ddlBENEFICIARYVISITER.SelectedValue & "','" & _
                            mySetting.ConvertDateToDecimal(Trim(txtVISITDATE.Text), Session("Company").ToString, Session("Module").ToString) & "','" & _
                            txtREASON.Text & "'," & txtENTITLEMENTAMT.Text & "," & txtUPTODATE_BALANCE.Text & ",'" & txtAUTOAPPROVALNO.Text & "'," & _
                            txtCLAIMAMT.Text & ",'" & ddlTYPE.SelectedValue & "','" & ddlSTATUS.SelectedValue & "','" & txtBILLNO.Text & "','" & _
                            mySetting.ConvertDateToDecimal(Trim(txtBILLDATE.Text), Session("Company").ToString, Session("Module").ToString) & _
                            "'," & txtBILLAMT.Text & ",'" & mySetting.ConvertDateToDecimal(Trim(txtPAYMENTMONTH.Text), Session("Company").ToString, Session("Module").ToString) & "','" & _
                            txtBILLGSTAMT.Text & "','" & txtBILLB4GSTAMT.Text & "','" & txtBILLGSTNO.Text & "','" & ddlCustomerName.SelectedValue & "','" & ddlInvoiceType.SelectedValue & "','" & _
                            Session("EmpID").ToString & "','CLAIM_ADD'"
                ...
                Else
                    ssql1 = "exec sp_md_InsUpdDelMedicalTransaction '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & _
                            ...
                            mySetting.ConvertDateToDecimal(Trim(txtBILLDATE.Text), Session("Company").ToString, Session("Module").ToString) & _
                            "'," & txtBILLAMT.Text & ",'" & _
                            mySetting.ConvertDateToDecimal(Trim(txtPAYMENTMONTH.Text), Session("Company").ToString, Session("Module").ToString) & "','" & _
                            txtBILLGSTAMT.Text & "','" & txtBILLB4GSTAMT.Text & "','" & txtBILLGSTNO.Text & "','" & ddlCustomerName.SelectedValue & "','" & ddlInvoiceType.SelectedValue & "','" & _
                            Session("EmpID").ToString & "','CLAIM_UPD'"
                    mySQL.ExecuteSQL(ssql1, Session("Company").ToString, Session("EmpID").ToString)
                    ProcessCompleted()
                End If
                ssql1 = Nothing
        ...
        ssql = Nothing
        myDS = Nothing
        txtENTITLEMENTAMT.Enabled = True
        txtUPTODATE_BALANCE.Enabled = True
    ...
    Protected Sub txtBILLB4GSTAMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtBILLB4GSTAMT.TextChanged
        If txtBILLB4GSTAMT.Text <> "" Then
            If CStr(txtBILLB4GSTAMT.Text * 0.06).IndexOf(".") > 0 Then
                txtBILLGSTAMT.Text = Mid(CStr(txtBILLB4GSTAMT.Text * 0.06), 1, CStr(txtBILLB4GSTAMT.Text * 0.06).IndexOf(".")) & "." & Mid(CStr(txtBILLB4GSTAMT.Text * 0.06), CStr(txtBILLB4GSTAMT.Text * 0.06).IndexOf(".") + 2, 2)
            Else
                txtBILLGSTAMT.Text = CStr(txtBILLB4GSTAMT.Text * 0.06)
            End If
            txtBILLAMT.Text = (CDbl(txtBILLGSTAMT.Text) + CDbl(txtBILLB4GSTAMT.Text))
        End If
    End Sub
End Class
