...
Partial Class Pages_MEDICAL_MEDICAL_CHIT_UPDATEBILL
...
#End Region
    ...
    Private Sub PagePreload()
        ...
        DisableField()
        mySetting.GetLookupValue_ImageButton(imgbtnAUTOAPPROVALNO, Form.ID, "txtAUTOAPPROVALNO", "Code", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "AUTOAPPROVALNO" & """," & """" & """," & """" & Session("EmpID").ToString & """")
    ...
    Private Sub DisableField()
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("SELECT Code,Option_Primary_Key,Option_Data_Type,Option_Mandatory FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' AND Option_View_Card='YES' and Code not in ('CLAIMAMT','BILLNO','BILLDATE','BILLAMT','PAYMENTMONTH','AUTOAPPROVALNO','STATUS','EMPLOYEE_PROFILE_ID') ORDER BY SEQUENCE_NO")
        For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
            Dim myImageKey As ImageButton = Page.FindControl("imgKey" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
            Dim myImageButton As ImageButton = Page.FindControl("imgbtn" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
            Dim myLabel As Label = Page.FindControl("lbl" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
    ...
    Protected Sub txtEMPLOYEE_PROFILE_ID_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtEMPLOYEE_PROFILE_ID.TextChanged
        ...
        mySetting.GetLookupValue_ImageButton(imgbtnAUTOAPPROVALNO, Form.ID, "txtAUTOAPPROVALNO", "Code", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "AUTOAPPROVALNO" & """," & """" & """," & """" & txtEMPLOYEE_PROFILE_ID.Text & """")
    End Sub
    ...
    Protected Sub txtAUTOAPPROVALNO_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtAUTOAPPROVALNO.TextChanged
        ssql = "Exec sp_md_SelMedicalChit 'Chit','" & txtAUTOAPPROVALNO.Text & "'"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            ...
            txtBENEFICIARYVISITER.Text = myDS.Tables(0).Rows(0).Item(4).ToString
    ...
    Protected Sub imgBtnUpdate_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdate.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ...
                myDS.Tables(0).Rows(0).Item(1).ToString & """,""" & myDS.Tables(0).Rows(0).Item(2).ToString & """,'',""" & _
                txtBENEFICIARYVISITER.Text & """,'','','','',""" & txtAUTOAPPROVALNO.Text & """," & txtCLAIMAMT.Text & ",'',""" & ddlSTATUS.SelectedValue & """,""" & _
                txtBILLNO.Text & """,""" & mySetting.ConvertDateToDecimal(Trim(txtBILLDATE.Text), Session("Company").ToString, Session("Module").ToString) & _
                """," & txtBILLAMT.Text & ",""" & _
                mySetting.ConvertDateToDecimal(Trim(txtPAYMENTMONTH.Text), Session("Company").ToString, Session("Module").ToString) & """,""" & _
                Session("EmpID").ToString & """,'CHIT_UPD'"
