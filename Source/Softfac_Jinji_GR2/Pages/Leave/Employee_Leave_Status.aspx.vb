...
Partial Class Pages_Leave_Employee_Leave_Status
...
#Region "Page Setting"
    ...
    Sub PagePreload()
        ...
        'lookup field seting
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "EMPLOYEE_PROFILE_ID" & """," & """" & """," & """" & Session("Module").ToString & """"
        mySetting.GetLookupValue_ImageButton(imgBtnEMPLOYEE_PROFILE_ID, Form.ID, "txtEMPLOYEE_PROFILE_ID", "CodeName", ssql)
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID_LEAVE" & """," & """" & """," & """" & Session("Module").ToString & """"
        mySetting.GetLookupValue_ImageButton(imgBtnOCP_ID_LEAVE, Form.ID, "txtOCP_ID_LEAVE", "CodeName", ssql)
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "USER_PROFILE_CODE_CREATE" & """," & """" & """," & """" & Session("Module").ToString & """"
                                    ...
                                    For Me.j = 0 To arr.Count - 1
                                        If Trim(arr(j).ToString) <> "" Then
                                            If Not IsNumeric(Trim(arr(j).ToString)) Then
                                                'Change Search Numeric End                                            lblresult2.Text = "Invalid Input [Numeric] Format For " & myDR(1).ToString & " With Value " & arr(j).ToString
                                                myTxt.Focus()
                                                Return False
                                            Else
