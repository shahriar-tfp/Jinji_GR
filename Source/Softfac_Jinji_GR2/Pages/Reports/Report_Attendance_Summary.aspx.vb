...
Partial Class Pages_Reports_Report_Attendance_Summary
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "OPTION_CITIZENSHIP" Then
                    If lblOPTION_CITIZENSHIP.Visible = True And ddlOPTION_CITIZENSHIP.Visible = True Then
                        autonum += 1
                        imgKeyOPTION_CITIZENSHIP.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyOPTION_CITIZENSHIP.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyOPTION_CITIZENSHIP.Style.Add("position", "absolute")
                        autonum += 1
                        lblOPTION_CITIZENSHIP.CssClass = "wordstyle12"
                        lblOPTION_CITIZENSHIP.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblOPTION_CITIZENSHIP.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblOPTION_CITIZENSHIP.Style.Add("position", "absolute")
                        autonum += 1
                        ddlOPTION_CITIZENSHIP.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        ddlOPTION_CITIZENSHIP.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        ddlOPTION_CITIZENSHIP.Style.Add("position", "absolute")
                        ddlOPTION_CITIZENSHIP.Width = ddlWidth
                        autonum += 1
                        imgbtnOPTION_CITIZENSHIP.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnOPTION_CITIZENSHIP.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnOPTION_CITIZENSHIP.Style.Add("position", "absolute")
                    End If
                End If
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        Try
            ...
            ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_datepart('month',dbo.fn_UnDisplayDate('" & txtDATE_FROM.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtDATE_FROM.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),@@spid"
            myDS = New DataSet()
            ...
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    ...
                    For k = 0 To itemCount - 1
                        ...
                        ssql1 = Nothing
                    Next
                    ssql1 = "exec temp_processattsummary2 '" & Session("Company").ToString & "','" & Session("EmpID").ToString & "','" & ddlOPTION_GROUP.SelectedValue.ToString & "','" & spid & "','" & mySetting.ConvertDateToDecimal(txtDATE_FROM.Text, Session("Company").ToString, Session("Module").ToString) & "','" & mySetting.ConvertDateToDecimal(txtDATE_TO.Text, Session("Company").ToString, Session("Module").ToString) & "','" & ddlOPTION_PAY_TYPE.SelectedValue & "','" & ddlOPTION_CITIZENSHIP.SelectedValue & "'"
                    myDS1 = mySQL.ExecuteSQL(ssql1)

                    If myDS1.Tables.Count > 0 Then
                        spid = myDS1.Tables(0).Rows(0).Item(0).ToString
                    Else
                        spid = ""
                    End If
                    myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&spid=" & spid & "&FromDate=" & txtDATE_FROM.Text & "&ToDate=" & txtDATE_TO.Text & "&rdTemplate=RPT_Attendance_Summary"
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        If myds.Tables(0).Rows(0).Item(0).ToString = "" Then
            ...
            Return False
        ElseIf IsNumeric(mySetting.ConvertDateToDecimal(txtDATE_FROM.Text, Session("Company").ToString, Session("Module").ToString)) = False Then
        ...
        ElseIf ddlOPTION_PAY_TYPE.SelectedValue = "" Then
            ...
            Return False
        ElseIf ddlOPTION_CITIZENSHIP.SelectedValue = "" Then
            lblresult2.Text = "[" & lblOPTION_CITIZENSHIP.Text & "] Is A Required Field!"
            ddlOPTION_CITIZENSHIP.Focus()
            Return False
