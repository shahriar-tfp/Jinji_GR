...
Partial Class PAGES_PAYROLL_EMPLOYEE_PAYROLL_ANALYSIS
                ...
                For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                    ...
                    Select Case myDR(2).ToString
                        ...
                        Case "CHARACTER", "LOOKUP"
                            ...
                            '        ssql2 = ssql2 & myDR(0).ToString & "@@"
                            '        ssql3 = ssql3 & Trim(myTxt.Text.ToString.Replace("'", "''")) & "@@"
                            '        If myDR(0).ToString = "EMPLOYEE_PROFILE_ID" Then
                            '            myID = "ISNULL(dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myTxt.Text.ToString & "'),'')"
                            '        Else
                            '            Dim myDST As New DataSet
                            '            myDST = mySQL.ExecuteSQL("Select Function_Name,Default_Value,Default_Value2 From User_Define_Function Where Company_Profile_Code='" & Session("Company").ToString & "' And Module_Profile_Code='" & Session("Module").ToString & "' And Table_Profile_Code='" & Form.ID & "' And Table_Field_Code='" & myDS.Tables(0).Rows(i).Item(0).ToString & "' And Query_Action='UPDATE'")
                            '            If myDST.Tables(0).Rows.Count > 0 Then
                            '                If myDST.Tables(0).Rows(0).Item(1).ToString.Trim = "" Then
                            '                    myID = "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTxt.Text.ToString & "'),'')"
                            '                Else
                            '                    myID = "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTxt.Text.ToString & "',N'" & myDST.Tables(0).Rows(0).Item(1).ToString.Trim & "'),'')"
                            '                End If
                            '            Else
                            '                myID = "ISNULL(N'" & myTxt.Text.ToString & "','')"
                            '            End If
                            '            myDST = Nothing
                            '        End If
                            '        Dim myDS2 As New DataSet
                            '        myDS2 = mySQL.ExecuteSQL("select " + myID)
                            '        If myDS2.Tables(0).Rows.Count > 0 Then
                            '            htbFilterCriteria.Add(myDR(0).ToString.Trim, myDS2.Tables(0).Rows(0).Item(0).ToString)
                            '        End If
                            '        myDS2 = Nothing
                            '    End If
                    End Select
