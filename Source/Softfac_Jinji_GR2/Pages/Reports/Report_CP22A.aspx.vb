...
Partial Class Pages_Reports_Report_CP22A
...
#Region "Panel Edit"
    ...
    Protected Sub imgbtnGROUP_CODE2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnGROUP_CODE2.Click
        ...
        lstleft.Items.Clear()
        lstright.Items.Clear()
        ssql = "Exec sp_sa_getListBoxValue 'Payslip','" & Session("Company") & "','" & ddlOPTION_GROUP.SelectedValue & "','" & """" & "','" & """" & "','','',''"
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        If itemCount > 0 Then
            For Me.k = 0 To itemCount - 1
                ssql1 = "exec sp_pr_InsUpdDelEmployeePayrollProcessTemp '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & lstright.Items(k).Value & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "',''," & spid & ",'DEL'"
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                For Me.k = 0 To itemCount - 1
                    ssql1 = "exec sp_pr_InsUpdDelEmployeePayrollProcessTemp '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & lstright.Items(k).Value & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "',''," & spid & ",'" & ddlOPTION_GROUP.SelectedValue & "'"
                    myDS1 = New DataSet()
                    myDS1 = mySQL.ExecuteSQL(ssql1)
                    myDS1 = Nothing
                    ssql1 = Nothing
                Next
                myTargetURL = myTargetURL & "&RegNo=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&PIC=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&SPID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&rdTemplate=pr_CP22A"
