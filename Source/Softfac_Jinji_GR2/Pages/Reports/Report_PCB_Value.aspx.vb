...
Partial Class Pages_Reports_Report_PCB_Value
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        ssql = "select dbo.fn_ReturnEmpIDByCodeName('" & Session("Company").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text & "'),dbo.fn_datepart('month',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "')),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "')),@@spid"
        myDS = New DataSet()
        myDS = mySQL.ExecuteSQL(ssql)
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                myTargetURL = myTargetURL & "&Employee_Profile_ID=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&rdTemplate=PCB_CAL"
