...
Partial Class Pages_Reports_Report_E_EA
...
#End Region
    ...
    Private Sub PagePreload()
        ...
        mySetting.GetImgUrl(imgKeyBONUS1, clsGlobalSetting.ImageType._KEY, Session("Company").ToString)
        mySetting.GetImgUrl(imgKeyBONUS2, clsGlobalSetting.ImageType._KEY, Session("Company").ToString)
        mySetting.GetImgUrl(imgbtnYEAR_MONTH, clsGlobalSetting.ImageType._DATE, Session("Company").ToString)
        mySetting.GetImgUrl(imgbtnBONUS1, clsGlobalSetting.ImageType._DATE, Session("Company").ToString)
        mySetting.GetImgUrl(imgbtnBONUS2, clsGlobalSetting.ImageType._DATE, Session("Company").ToString)
        ...
        If myDS.Tables.Count > 1 Then
            ...
            If myDT1.Rows.Count > 0 Then
                ...
                Else
                    ...
                    ShowMessage("You are not allowed to view this page!")
                    Exit Sub
                End If
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),@@spid"
        ...
        If itemCount > 0 Then
            ssql1 = "exec sp_pr_InsUpdDelEmployeePayrollProcessTemp '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "',''," & spid & ",'DEL'"
        End If
        ...
        myDS1 = Nothing
        ssql1 = Nothing
        ssql2 = "Select ReportName from REPORT_FORMAT where table_profile_code = '" & Form.ID & "' and " & myDS.Tables(0).Rows(0).Item(1).ToString & " between From_Year and To_Year and ReportCode ='EA-SP'"
        ...
        If myDS2.Tables.Count > 0 Then
            If myDS2.Tables(0).Rows.Count > 0 Then
                ssql3 = "Exec " & myDS2.Tables(0).Rows(0).Item(0).ToString & " '"
            Else
                ssql3 = "Exec [sp_pr_InsDelTempEAForm] '"
            End If
        Else
            ssql3 = "Exec [sp_pr_InsDelTempEAForm] '"
        End If
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                For k = 0 To itemCount - 1
                    ssql1 = ssql3 & ddlOPTION_GROUP.SelectedValue & "','" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & lstright.Items(k).Value & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "'"
                    myDS1 = New DataSet()
                    myDS1 = mySQL.ExecuteSQL(ssql1)
                    myDS1 = Nothing
                    ssql1 = Nothing
                    ...
                    ssql1 = Nothing
                Next
                'myTargetURL = myTargetURL & "&RegNo=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&SpID=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&bonus1=" & txtBONUS1.Text & "&bonus2=" & txtBONUS2.Text & "&User_Profile_Code=" & Session("EmpID").ToString & "&rdTemplate=pr_GovEEA"
                ssql2 = "Select ReportName from REPORT_FORMAT where table_profile_code = '" & Form.ID & "' and " & myDS.Tables(0).Rows(0).Item(1).ToString & " between From_Year and To_Year and ReportCode ='EA-Form'"
                myDS2 = mySQL.ExecuteSQL(ssql2, Session("Company").ToString, Session("EmpID").ToString)

                myTargetURL = myTargetURL & "&RegNo=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&SpID=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&bonus1=" & txtBONUS1.Text & "&bonus2=" & txtBONUS2.Text & "&User_Profile_Code=" & Session("EmpID").ToString & "&rdTemplate="
                If myDS2.Tables.Count > 0 Then
                    If myDS2.Tables(0).Rows.Count > 0 Then
                        myTargetURL = myTargetURL & myDS2.Tables(0).Rows(0).Item(0).ToString
                    Else
                        myTargetURL = myTargetURL & "pr_GovEEA"
                    End If
                Else
                    myTargetURL = myTargetURL & "pr_GovEEA"
                End If
                ...
                myTargetURL = clsRpt.GetPDFReportURL()
                'myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&SpID=" & spid & "&Location=PENINSULA&Type=" & ddlOPTION_GROUP.SelectedValue & "&User_Profile_Code=" & Session("EmpID").ToString & "&rdTemplate=pr_GOVEPCBII"
                ssql2 = "Select ReportName from REPORT_FORMAT where table_profile_code = '" & Form.ID & "' and " & myDS.Tables(0).Rows(0).Item(1).ToString & " between From_Year and To_Year and ReportCode ='EPCBII'"
                myDS2 = mySQL.ExecuteSQL(ssql2, Session("Company").ToString, Session("EmpID").ToString)

                myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&SpID=" & spid & "&Location=PENINSULA&Type=" & ddlOPTION_GROUP.SelectedValue & "&User_Profile_Code=" & Session("EmpID").ToString & "&rdTemplate="
                If myDS2.Tables.Count > 0 Then
                    If myDS2.Tables(0).Rows.Count > 0 Then
                        myTargetURL = myTargetURL & myDS2.Tables(0).Rows(0).Item(0).ToString
                    Else
                        myTargetURL = myTargetURL & "pr_GOVEPCBII"
                    End If
                Else
                    myTargetURL = myTargetURL & "pr_GOVEPCBII"
                End If
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "BONUS1" Then
                    If lblBONUS1.Visible = True And txtBONUS1.Visible = True Then
                        autonum += 1
                        imgKeyBONUS1.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyBONUS1.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyBONUS1.Style.Add("position", "absolute")
                        autonum += 1
                        lblBONUS1.CssClass = "wordstyle12"
                        lblBONUS1.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblBONUS1.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblBONUS1.Style.Add("position", "absolute")
                        autonum += 1
                        txtBONUS1.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        txtBONUS1.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        txtBONUS1.Style.Add("position", "absolute")
                        txtBONUS1.Width = txtWidth
                        autonum += 1
                        imgbtnBONUS1.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnBONUS1.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnBONUS1.Style.Add("position", "absolute")
                    End If
                End If

                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "BONUS2" Then
                    If lblBONUS2.Visible = True And txtBONUS2.Visible = True Then
                        autonum += 1
                        imgKeyBONUS2.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyBONUS2.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyBONUS2.Style.Add("position", "absolute")
                        autonum += 1
                        lblBONUS2.CssClass = "wordstyle12"
                        lblBONUS2.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblBONUS2.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblBONUS2.Style.Add("position", "absolute")
                        autonum += 1
                        txtBONUS2.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        txtBONUS2.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        txtBONUS2.Style.Add("position", "absolute")
                        txtBONUS2.Width = txtWidth
                        autonum += 1
                        imgbtnBONUS2.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnBONUS2.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnBONUS2.Style.Add("position", "absolute")
                    End If
                End If
