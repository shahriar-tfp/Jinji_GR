...
Partial Class Pages_Reports_Report_Attendance_ByDate
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        Try
            ...
            ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_UnDisplayDate('" & txtDATE.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "'),@@spid"
            myDS = New DataSet()
            ...
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    ...
                    For k = 0 To itemCount - 1
                        ...
                        ssql1 = Nothing
                    Next
                    myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Date_From=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&Date_To=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&User_Profile_Code=" & Session("EmpID").ToString & "&rdTemplate=Roster_Attendance_BYDate"
