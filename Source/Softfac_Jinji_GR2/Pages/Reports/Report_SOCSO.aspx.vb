...
Partial Class Pages_Reports_Report_SOCSO
...
#End Region
    ...
    Private Sub PagePreload()
        ...
        mySetting.GetBtnImgUrl(imgBtnPrintSOCSOA, Session("Company").ToString, btnColourDef, "btnPrint.png")
        ...
        If myDS.Tables.Count > 1 Then
            ...
            If myDT1.Rows.Count > 0 Then
                ...
                imgBtnPrintSOCSOA.Visible = AllowPrint
            End If
        ...
        imgBtnPrintSOCSOA.CssClass = buttonPosition3
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_datepart('month',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),@@spid,dbo.fn_getOCPIDBYCODENAME(N'" & txtOCP_ID.Text & "','DIVISION')"
        myDS = New DataSet()
        myDS = mySQL.ExecuteSQL(ssql)
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                If ddlOPTION_SOCSO_TYPE.SelectedValue = "SOCSO_8A" Then
                    myTargetURL = myTargetURL & "&OrganID=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&SPID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&Type=ADD&paytype=" & ddlOPTION_PAY_TYPE.SelectedValue & "&Location=" & ddlOPTION_LOCATION.SelectedValue & "&Module=" & Session("Module").ToString & "&userID=" & Session("EmpID").ToString & "&Cheque=" & txtCHEQUE.Text.ToUpper & "&PICName=" & txtPIC_NAME.Text.ToUpper & "&PICNRIC=" & txtPIC_NRIC.Text.ToUpper & "&PICPosition=" & txtPIC_POSITION.Text.ToUpper & "&PICTelNo=" & txtPIC_TELNO.Text.ToUpper & "&Page=" & ddlOPTION_PAGE.SelectedValue & "&OCP_ID=" & myDS.Tables(0).Rows(0).Item(4).ToString & "&rdTemplate=pr_GovSOCSO"
                    Response.Write("<script>window.open('" & myTargetURL & "');</script>")
                ElseIf ddlOPTION_SOCSO_TYPE.SelectedValue = "REG_SOCSO" Then
                    myTargetURL = myTargetURL & "&OrganID=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&SPID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&Type=BRG2&paytype=" & ddlOPTION_PAY_TYPE.SelectedValue & "&Location=" & ddlOPTION_LOCATION.SelectedValue & "&Module=" & Session("Module").ToString & "&userID=" & Session("EmpID").ToString & "&rdTemplate=pr_GovBRG2"
                    Response.Write("<script>window.open('" & myTargetURL & "');</script>")
                ElseIf ddlOPTION_SOCSO_TYPE.SelectedValue = "DEREG_SOCSO" Then
                    myTargetURL = myTargetURL & "&OrganID=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&SPID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&Type=BRG3&paytype=" & ddlOPTION_PAY_TYPE.SelectedValue & "&Location=" & ddlOPTION_LOCATION.SelectedValue & "&Module=" & Session("Module").ToString & "&userID=" & Session("EmpID").ToString & "&rdTemplate=pr_GovBRG3"
                    Response.Write("<script>window.open('" & myTargetURL & "');</script>")
                End If
            End If
        End If
    ...
    Protected Sub imgBtnSaveAs_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSaveAs.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ssql1 = "Exec sp_pr_SelSOCSO 'SOCSODETAIL','" & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & "," & myDS.Tables(0).Rows(0).Item(2).ToString & ",'" & myDS.Tables(0).Rows(0).Item(4).ToString & "'"
                myDS1 = New DataSet
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        If myds.Tables(0).Rows(0).Item(0).ToString = "" Then
            ...
            Return False
        ElseIf ddlOPTION_SOCSO_TYPE.SelectedValue = "" Then
            lblresult2.Text = "[" & lblOPTION_SOCSO_TYPE.Text & "] Is A Required Field!"
            ddlOPTION_SOCSO_TYPE.Focus()
            Return False
    ...
    Protected Sub imgBtnPrintSOCSOA_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrintSOCSOA.Click
        Dim clsRpt As New clsReportServices, myTargetURL As String = ""
        myTargetURL = clsRpt.GetPDFReportURL()
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_datepart('month',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),@@spid,dbo.fn_getOCPIDBYCODENAME(N'" & txtOCP_ID.Text & "','DIVISION')"
        myDS = New DataSet()
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&Option_Location=" & ddlOPTION_LOCATION.SelectedValue & "&Cheque=" & txtCHEQUE.Text.ToUpper & "&PICName=" & txtPIC_NAME.Text.ToUpper & "&PICNRIC=" & txtPIC_NRIC.Text.ToUpper & "&PICPosition=" & txtPIC_POSITION.Text.ToUpper & "&PICTelNo=" & txtPIC_TELNO.Text.ToUpper & "&OCP_ID=" & myDS.Tables(0).Rows(0).Item(4).ToString & "&rdTemplate=pr_GovSOCSOA"
                Response.Write("<script>window.open('" & myTargetURL & "');</script>")
            End If
        End If
        ssql = Nothing
