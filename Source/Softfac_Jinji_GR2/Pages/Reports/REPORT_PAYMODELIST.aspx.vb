...
Partial Class Pages_Reports_REPORT_PAYMODELIST
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "OPTION_PAYMODE" Then
                    If lblOption_Paymode.Visible = True And ddlOption_Paymode.Visible = True Then
                        autonum += 1
                        imgKeyOption_Paymode.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyOption_Paymode.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyOption_Paymode.Style.Add("position", "absolute")
                        autonum += 1
                        lblOption_Paymode.CssClass = "wordstyle12"
                        lblOption_Paymode.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblOption_Paymode.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblOption_Paymode.Style.Add("position", "absolute")
                        autonum += 1
                        ddlOption_Paymode.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        ddlOption_Paymode.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        ddlOption_Paymode.Style.Add("position", "absolute")
                        ddlOption_Paymode.Width = txtWidth
                        autonum += 1
                        imgbtnOption_Paymode.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnOption_Paymode.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnOption_Paymode.Style.Add("position", "absolute")
                    End If
                End If
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        Try
            ...
            ssql = "select dbo.fn_GetEmpID(N'" & Session("EmpID").ToString & "'),dbo.fn_datepart('month',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',N'" & Session("Company").ToString & "','" & Session("Module").ToString & "')),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',N'" & Session("Company").ToString & "','" & Session("Module").ToString & "')),@@spid"
            myDS = New DataSet()
            ...
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    ...
                    ssql1 = Nothing
                    'Next
                    myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&Month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&Spid=" & spid & "&Option_Pay_Mode=" & ddlOption_Paymode.SelectedValue & "&Date=" & txtPAY_DATE.Text & "&Cheque=" & txtCHEQUE.Text & "&ByRpt=" & ddlOPTION_REPORT.SelectedValue & "&rdTemplate=pr_PayModeList"
