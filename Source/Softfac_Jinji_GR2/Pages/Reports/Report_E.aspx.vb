...
Partial Class Pages_Reports_Report_E
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),dbo.fn_ReturnEmpIDByCodeName( dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'), N'" & txtEMPLOYEE_PROFILE_ID.Text & "'),dbo.fn_getOCPIDBYCODENAME(N'" & txtOCP_ID.Text & "','DIVISION')"

        myDS = New DataSet()
        myDS = mySQL.ExecuteSQL(ssql)
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ssql1 = "Select ReportName from REPORT_FORMAT where table_profile_code = '" & Form.ID & "' and " & myDS.Tables(0).Rows(0).Item(1).ToString & " between From_Year and To_Year and ReportCode ='E-FORM'"
                myDS1 = mySQL.ExecuteSQL(ssql1, Session("Company").ToString, Session("EmpID").ToString)

                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&PicID=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&OCP_ID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&rdTemplate="
                If myDS1.Tables.Count > 0 Then
                    If myDS1.Tables(0).Rows.Count > 0 Then
                        myTargetURL = myTargetURL & myDS1.Tables(0).Rows(0).Item(0).ToString
                    Else
                        myTargetURL = myTargetURL & "pr_GovEForm"
                    End If
                Else
                    myTargetURL = myTargetURL & "pr_GovEForm"
                End If
                ...
                myTargetURL = clsRpt.GetPDFReportURL()
                ssql1 = "Select ReportName from REPORT_FORMAT where table_profile_code = '" & Form.ID & "' and " & myDS.Tables(0).Rows(0).Item(1).ToString & " between From_Year and To_Year and ReportCode ='CP8D'"
                myDS1 = mySQL.ExecuteSQL(ssql1, Session("Company").ToString, Session("EmpID").ToString)
                'myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&PicID=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&OCP_ID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&rdTemplate=pr_GovCP8D"
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&PicID=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&OCP_ID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&rdTemplate="
                If myDS1.Tables.Count > 0 Then
                    If myDS1.Tables(0).Rows.Count > 0 Then
                        myTargetURL = myTargetURL & myDS1.Tables(0).Rows(0).Item(0).ToString
                    Else
                        myTargetURL = myTargetURL & "pr_GovCP8D"
                    End If
                Else
                    myTargetURL = myTargetURL & "pr_GovCP8D"
                End If
        ...
        'Response.Write("<script>window.open('" & myTargetURL & "');</script>")
    End Sub
    Protected Sub imgBtnSaveAs_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSaveAs.Click
        Dim clsRpt As New clsReportServices, myTargetURL As String = "", myDS3 As DataSet
        myTargetURL = clsRpt.GetPDFReportURL()
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                myDS1 = Nothing
                ssql1 = Nothing
                ssql3 = "Select ReportName from REPORT_FORMAT where table_profile_code = '" & Form.ID & "' and " & myDS.Tables(0).Rows(0).Item(1).ToString & " between From_Year and To_Year and ReportCode ='E-CP8D'"
                myDS3 = mySQL.ExecuteSQL(ssql3, Session("Company").ToString, Session("EmpID").ToString)

                If myDS3.Tables.Count > 0 Then
                    If myDS3.Tables(0).Rows.Count > 0 Then
                        ssql1 = "Exec " & myDS3.Tables(0).Rows(0).Item(0).ToString & " '"
                        ssql2 = "Exec " & myDS3.Tables(0).Rows(0).Item(0).ToString & " '"
                    Else
                        ssql1 = "Exec [sp_pr_rptE8D] '"
                        ssql2 = "Exec [sp_pr_rptE8D] '"
                    End If
                Else
                    ssql1 = "Exec [sp_pr_rptE8D] '"
                    ssql2 = "Exec [sp_pr_rptE8D] '"
                ...
                ssql2 = ssql2 & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & ",'" & myDS.Tables(0).Rows(0).Item(2).ToString & "','','" & myDS.Tables(0).Rows(0).Item(3).ToString & "','TXT1'"
                myDS1 = New DataSet
                myDS2 = mySQL.ExecuteSQL(ssql2)
                'ssql1 = "Exec [sp_pr_rptE8D] '" & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & ",'" & myDS.Tables(0).Rows(0).Item(2).ToString & "','','" & myDS.Tables(0).Rows(0).Item(3).ToString & "','TXT1'"
                'myDS1 = New DataSet
                'myDS1 = mySQL.ExecuteSQL(ssql1)
                Response.ContentType = "Text/plain"
                Response.AppendHeader("Content-Disposition", "attachment; filename=P" & myDS2.Tables(0).Rows(0).Item(0).ToString & "_" & myDS.Tables(0).Rows(0).Item(1).ToString & ".txt")
                ...
                myDS1 = Nothing
                'ssql1 = Nothing
                ssql1 = ssql1 & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & ",'" & myDS.Tables(0).Rows(0).Item(2).ToString & "','','" & myDS.Tables(0).Rows(0).Item(3).ToString & "','TXT2'"
                myDS1 = New DataSet
                myDS1 = mySQL.ExecuteSQL(ssql1)
                For i = 0 To myDS1.Tables(0).Rows.Count - 1
                    str.Append(myDS1.Tables(0).Rows(i).Item(0).ToString)
