...
Partial Class Pages_Reports_Report_SIP
...
#End Region
    ...
    Private Sub PagePreload()
        ...
        mySetting.GetBtnImgUrl(imgBtnPrintSIPA, Session("Company").ToString, btnColourDef, "btnPrint.png")
        ...
        If myDS.Tables.Count > 1 Then
            ...
            If myDT1.Rows.Count > 0 Then
                ...
                imgBtnPrintSIPA.Visible = AllowPrint
            End If
        ...
        imgBtnPrintSIPA.CssClass = buttonPosition3
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ...
                ElseIf ddlOPTION_SIP_TYPE.SelectedValue = "REG_SIP" Then
                    ...
                    Response.Write("<script>window.open('" & myTargetURL & "');</script>")
                ElseIf ddlOPTION_SIP_TYPE.SelectedValue = "DEREG_SIP" Then
    ...
    Protected Sub imgBtnSaveAs_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSaveAs.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ssql1 = "Exec sp_pr_SelSIP 'SIPDETAIL','" & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & "," & myDS.Tables(0).Rows(0).Item(2).ToString & ",'" & myDS.Tables(0).Rows(0).Item(4).ToString & "'"
                myDS1 = New DataSet
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        If myds.Tables(0).Rows(0).Item(0).ToString = "" Then
            ...
            Return False
        ElseIf ddlOPTION_SIP_TYPE.SelectedValue = "" Then
            lblresult2.Text = "[" & lblOPTION_SIP_TYPE.Text & "] Is A Required Field!"
            ddlOPTION_SIP_TYPE.Focus()
            Return False
    ...
    Protected Sub imgBtnPrintSIPA_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrintSIPA.Click
        Dim clsRpt As New clsReportServices, myTargetURL As String = ""
        myTargetURL = clsRpt.GetPDFReportURL()
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_datepart('month',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),dbo.fn_datepart('year',dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "')),@@spid,dbo.fn_getOCPIDBYCODENAME(N'" & txtOCP_ID.Text & "','DIVISION')"
        myDS = New DataSet()
