...
Partial Class PAGES_INFORMATION_SYSTEM_EMPLOYEE_CLAIM
    ...
    Protected Sub imgBtnAdd_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnAdd.Click
        ...
        ElseIf CountRecord = 1 Then
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                End If
            Next
            ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "ALLOW_DETAILS" & """," & "" & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlALLOW_TYPE.SelectedValue & """"
            imgbtnALLOW_DETAILS.Attributes.Remove("onclick")

            mySetting.GetLookupValue_ImageButton(imgbtnALLOW_DETAILS, Form.ID, "txtALLOW_DETAILS", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "ALLOW_DETAILS" & """," & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlALLOW_TYPE.SelectedValue & """")
        ElseIf CountRecord > 1 Then
        ...
        If ActionValidateDelete() Then
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                If chkDelete.Checked Then
                    ...
                    '        myDS = New DataSet
                    '        'Dim abc As String
                    '        'abc = "SELECT Code,Option_Data_Type,Name FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' and Code = 'ALLOW_TYPE' ORDER BY SEQUENCE_NO"
                    '        myDS = mySQL.ExecuteSQL("SELECT Code,Option_Data_Type,Name FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' and Code = 'ALLOW_TYPE' ORDER BY SEQUENCE_NO", Session("Company").ToString, Session("EmpID").ToString)
                    '        For Me.j = 0 To myDS.Tables(0).Rows.Count - 1
                    '            For Me.intColumn = 1 To gvHistory.HeaderRow.Cells.Count - 1
                    '                If gvHistory.HeaderRow.Cells(intColumn).Text.ToString = myDS.Tables(0).Rows(j).Item(2).ToString Then
                    '                    Select Case myDS.Tables(0).Rows(j).Item(1).ToString.Trim
...
#Region "Sub & Function"
    ...
    Private Function ValidateInsert() As Boolean
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
                    ...
                    Case Else   '"CHARACTER", "LOOKUP"
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ...
                                ElseIf myDS.Tables(0).Rows(i).Item(0).ToString.ToUpper = "ALLOW_DETAILS" Then
                                    Dim myDST As New DataSet
                                    myDST = mySQL.ExecuteSQL("select left(code,10) from organisation_Code_Profile_vw where ID = '" & ddlALLOW_TYPE.SelectedValue & "'")
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        If myDST.Tables(0).Rows(0).Item(0).ToString.Trim = "ALLOWCLAIM" Then
                                            ssql2 = ssql2 & "ISNULL(dbo.fn_GetCode_ALLOWANCE_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),''),"
                                        Else
                                            ssql2 = ssql2 & "ISNULL(dbo.fn_GetCode_REIMBURSEMENT_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),''),"
                                        End If
                                        If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                            If myDST.Tables(0).Rows(0).Item(0).ToString.Trim = "ALLOWCLAIM" Then
                                                ssql3 = ssql3 & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(dbo.fn_GetCode_ALLOWANCE_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),0) And "
                                            Else
                                                ssql3 = ssql3 & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(dbo.fn_GetCode_REIMBURSEMENT_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),0) And "
                                            End If
                                        End If
                                    Else
                                        ssql2 = ssql2 & "'" & Trim(myTextBox.Text) & "',"
            ...
            If Not myTXT Is Nothing Then '
                ...
                ssql2 = ssql2 & "'" & Session("EmpID").ToString & "',dbo.fn_UnDisplayDateTime(dbo.fn_DisplayDateTime(dbo.fn_GetCurrentDateTime(GetDate()),'" & Session("company").ToString & "','" & Session("module").ToString & "'),'" & Session("company").ToString & "','" & Session("module").ToString & "'),'" & Session("EmpID").ToString & "',dbo.fn_UnDisplayDateTime(dbo.fn_DisplayDateTime(dbo.fn_GetCurrentDateTime(GetDate()),'" & Session("company").ToString & "','" & Session("module").ToString & "'),'" & Session("company").ToString & "','" & Session("module").ToString & "'),"
            End If
            ssql = ssql & "SEQUENCE_NO,"
            Dim newsql As String
            newsql = "select top 1 (sequence_no + 1) from EMPLOYEE_CLAIM where employee_profile_id = ISNULL(dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & txtEMPLOYEE_PROFILE_ID.Text.Trim & "'),'') and dbo.fn_DisplayDate(DATETIME_CREATE,'" & Session("Company").ToString & "','" & Session("Module").ToString & "') = dbo.fn_DisplayDate(dbo.fn_getCurrentDate(getdate()),'" & Session("Company").ToString & "','" & Session("Module").ToString & "') order by sequence_no desc"
            DefaultDS = mySQL.ExecuteSQL("select top 1 (sequence_no + 1) from EMPLOYEE_CLAIM where employee_profile_id = ISNULL(dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & txtEMPLOYEE_PROFILE_ID.Text.Trim & "'),'') and dbo.fn_DisplayDate(DATETIME_CREATE,'" & Session("Company").ToString & "','" & Session("Module").ToString & "') = dbo.fn_DisplayDate(dbo.fn_getCurrentDate(getdate()),'" & Session("Company").ToString & "','" & Session("Module").ToString & "') order by sequence_no desc")
            If DefaultDS.Tables.Count > 0 Then
                If DefaultDS.Tables(0).Rows.Count > 0 Then
                    ssql2 = ssql2 & DefaultDS.Tables(0).Rows(0).Item(0).ToString & ","
                    ssql3 = ssql3 & "SEQUENCE_NO=" & DefaultDS.Tables(0).Rows(0).Item(0).ToString & " And "
                Else
                    ssql2 = ssql2 & "1,"
                    ssql3 = ssql3 & "SEQUENCE_NO=1 And "
                End If
            End If
            DefaultDS = Nothing
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ...
                                ElseIf myDS.Tables(0).Rows(i).Item(0).ToString.ToUpper = "ALLOW_DETAILS" Then
                                    ...
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        If myDST.Tables(0).Rows(0).Item(0).ToString.Trim = "ALLOWCLAIM" Then
                                            ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(dbo.fn_GetCode_ALLOWANCE_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),''),"
                                        Else
                                            ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(dbo.fn_GetCode_REIMBURSEMENT_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),''),"
                                        End If
                                    Else
                                        ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(N'" & myTextBox.Text.ToString & "',''),"
                                    End If
                        ...
                        Case Else '"CHARACTER","LOOKUP"
                            ...
                            If myTextBox.Visible = True Then
                                If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                    ...
                                    ElseIf myDS2.Tables(0).Rows(i).Item(0).ToString.ToUpper = "ALLOW_DETAILS" Then
                                        Dim myDST As New DataSet
                                        myDST = mySQL.ExecuteSQL("select left(code,10) from organisation_Code_Profile_vw where ID = '" & ddlALLOW_TYPE.SelectedValue & "'")
                                        If myDST.Tables(0).Rows.Count > 0 Then
                                            If myDST.Tables(0).Rows(0).Item(0).ToString.Trim = "ALLOWCLAIM" Then
                                                ssql = ssql & myDS2.Tables(0).Rows(i).Item(0) & "=ISNULL(dbo.fn_GetCode_ALLOWANCE_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),'') And "
                                            Else
                                                ssql = ssql & myDS2.Tables(0).Rows(i).Item(0) & "=ISNULL(dbo.fn_GetCode_REIMBURSEMENT_CLAIM_PROFILE_Vw(N'" & myTextBox.Text.ToString & "'),'') And "
                                            End If
                                        Else
                                            ssql = ssql & myDS2.Tables(0).Rows(i).Item(0) & "=ISNULL(N'" & myTextBox.Text.ToString & "','') And "
                                        End If
    ...
    End Function
    Sub BindAllowType()
        ssql = "exec [sp_org_getOCPIDObject] 'ALLOWTYPE','" & Session("Company") & "','','','','','','',''"
        myDS2 = mySQL.ExecuteSQL(ssql)
        If myDS2.Tables.Count > 0 Then
            ddlALLOW_TYPE.DataTextField = "Name"
            ddlALLOW_TYPE.DataValueField = "Code"
            ddlALLOW_TYPE.DataSource = myDS2
            ddlALLOW_TYPE.DataBind()
        End If
        myDS2 = Nothing
    End Sub
#End Region
    ...
    Protected Sub ddlALLOW_TYPE_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlALLOW_TYPE.SelectedIndexChanged
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "ALLOW_DETAILS" & """," & "" & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlALLOW_TYPE.SelectedValue & """"
        imgbtnALLOW_DETAILS.Attributes.Remove("onclick")

        mySetting.GetLookupValue_ImageButton(imgbtnALLOW_DETAILS, Form.ID, "txtALLOW_DETAILS", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "ALLOW_DETAILS" & """," & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlALLOW_TYPE.SelectedValue & """")
        txtALLOW_DETAILS.Text = ""
    End Sub
End Class