...
Partial Class PAGES_INFORMATION_SYSTEM_IMPORT_EMPLOYEE_PROFILE
...
#Region "Public Declaration"
    ...
    Public ssql As String, ssql1 As String, i As Integer, j As Integer, SearchByPage As Boolean, RecFound As Boolean, CountRecord As Integer
...
#Region "Page Setting"
    ...
    Sub PagePreload()
        ...
        mySetting.GetBtnImgUrl(imgBtnUpload, Session("Company").ToString, btnColourDef, "btnUpload.png")
        mySetting.GetBtnImgUrl(imgBtnUpdateImport, Session("Company").ToString, btnColourDef, "btnUpdate.png")
        ...
        If myDS.Tables.Count > 1 Then
            ...
            mySetting.GetGridViewSetting(Session("Company").ToString, Session("Module").ToString, "No_Of_Record", Form.ID, "GetGridViewRecord", GVUpload)
...
#End Region
    ...
    End Sub
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                Dim postedFile As HttpPostedFile = txtFileName.PostedFile
                ...
                savePath = System.AppDomain.CurrentDomain.BaseDirectory & "Import\Company\" & Session("Company").ToString & "\"
                postedFile.SaveAs(savePath & filename)
                Dim strConn As String = "Provider=Microsoft.Jet.OLEDB.4.0;" & _
                "Data Source=" & savePath & filename & ";" & _
                "Extended Properties=Excel 8.0;"
                Dim myData As New OleDbDataAdapter("SELECT * FROM [Emp Personal$]", strConn)
                myData.TableMappings.Add("Table", "ExcelTest")
                myData.Fill(myDS)

                GVUpload.DataSource = myDS.Tables(0).DefaultView
                GVUpload.DataBind()

                If myDS.Tables(0).Rows.Count > 0 Then
                    For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                        If myDS.Tables(0).Rows(i).Item(1).ToString <> "" Then
                            ssql = "insert into Import_employee_profile values(N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(0).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(1).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(2).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(3).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(4).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(5).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(6).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(7).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(8).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(9).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(10).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(11).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(12).ToString + """,N"""
                            ssql = ssql + myDS.Tables(0).Rows(i).Item(13).ToString + """,dbo.fn_getcurrentdatetime(getdate()),"
                            ssql = ssql & "N'" & Session("EmpID").ToString & "',dbo.fn_getcurrentdatetime(getdate()),N'" & Session("EmpID").ToString & "')"
                            mySQL.ExecuteSQL(ssql)
                            ssql = Nothing
                        End If
                    Next
                End If
                'lblresult3.Text = postedFile.Filename & " uploaded" & _
                '  "<br>content type: " & contentType & _
                '  "<br>content length: " & contentLength.ToString()
            Catch exc As Exception
                lblresult3.Text = "Failed uploading file"
            End Try
        End If
    ...
    Private Sub CheckIfDirectoriesExists()
        If Directory.Exists(System.AppDomain.CurrentDomain.BaseDirectory & "Import\Company\" & Session("Company").ToString & "\") Then
            'Do nothing
        Else
            Directory.CreateDirectory(System.AppDomain.CurrentDomain.BaseDirectory & "Import\Company\" & Session("Company").ToString & "\")
        End If
    End Sub

    Protected Sub imgBtnUpdateImport_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdateImport.Click
        Try
            If GVUpload.Rows.Count > 0 Then
                For Me.i = 0 To GVUpload.Rows.Count - 1
                    ssql1 = "select count(*) from employee_profile where code = N'" & GVUpload.Rows(i).Cells(2).Text.Trim.ToUpper & "'"
                    Dim myds1 As DataSet
                    myds1 = mySQL.ExecuteSQL(ssql1)
                    If CInt(myds1.Tables(0).Rows(0).Item(0)) > 0 Then
                        GVUpload.Rows(i).Cells(0).Text = "Employee Code Already Been Use."
                    Else
                        ssql = "insert into Employee_Profile(Company_Profile_Code,Code,Identity_Card_No,Old_Identity_Card_No,Date_Of_Birth,Place_Of_Birth,Option_Religion,Option_Marital_Status,Option_Sex,Option_Citizenship,Option_Race,Datetime_Create,User_Profile_Code_Create,Datetime_Modify,User_Profile_Code_Modify)"
                        ssql = ssql & " Select N'" & GVUpload.Rows(i).Cells(1).Text.Trim.ToUpper.Replace("'", "''") & "',"
                        ssql = ssql & "N'" & GVUpload.Rows(i).Cells(2).Text.Trim.ToUpper.Replace("'", "''") & "',"
                        ssql = ssql & "replace(N'" & GVUpload.Rows(i).Cells(5).Text.Trim.ToUpper.Replace("'", "''") & "','&NBSP;',''),"
                        ssql = ssql & "replace(N'" & GVUpload.Rows(i).Cells(6).Text.Trim.ToUpper.Replace("'", "''") & "','&NBSP;',''),"
                        ssql = ssql & "isnull(N'" & mySetting.UnDisplayDateTime(Trim(GVUpload.Rows(i).Cells(8).Text.Trim.ToUpper.Replace("'", "''")), Session("Company").ToString, Session("Module").ToString) & "','19000101000000'),"
                        ssql = ssql & "replace(N'" & GVUpload.Rows(i).Cells(9).Text.Trim.ToUpper.Replace("'", "''") & "','&NBSP;',''),"
                        ssql = ssql & "DBO.FN_GETOPTIONCODE(N'Employee_Profile',N'OPTION_RELIGION',N'" & GVUpload.Rows(i).Cells(10).Text.Trim.ToUpper.Replace("'", "''") & "'),"
                        ssql = ssql & "DBO.FN_GETOPTIONCODE(N'Employee_Profile',N'OPTION_MARITAL_STATUS',N'" & GVUpload.Rows(i).Cells(11).Text.Trim.ToUpper.Replace("'", "''") & "'),"
                        ssql = ssql & "DBO.FN_GETOPTIONCODE(N'Employee_Profile',N'OPTION_SEX',N'" & GVUpload.Rows(i).Cells(12).Text.Trim.ToUpper.Replace("'", "''") & "'),"
                        ssql = ssql & "DBO.FN_GETOPTIONCODE(N'Employee_Profile',N'OPTION_CITIZENSHIP',N'" & GVUpload.Rows(i).Cells(13).Text.Trim.ToUpper.Replace("'", "''") & "'),"
                        ssql = ssql & "DBO.FN_GETOPTIONCODE(N'Employee_Profile',N'OPTION_RACE',N'" & GVUpload.Rows(i).Cells(14).Text.Trim.ToUpper.Replace("'", "''") & "'),dbo.fn_getcurrentdatetime(getdate()),"
                        ssql = ssql & "N'" & Session("EmpID").ToString & "',dbo.fn_getcurrentdatetime(getdate()),N'" & Session("EmpID").ToString & "'"
                        mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                        ssql = Nothing
                        ssql2 = "select ID from employee_profile where code = N'" & GVUpload.Rows(i).Cells(2).Text.Trim.ToUpper & "'"
                        ...
                        If myDS2.Tables.Count > 0 Then
                            If myDS2.Tables(0).Rows.Count > 0 Then
                                employee_id = myDS2.Tables(0).Rows(0).Item(0).ToString
                            End If
                        End If
                        If CInt(employee_id) > 0 Then
                            ssql = "insert into employee_name(Employee_Profile_ID,Effective_Date,Option_Title,Name) select N'" & employee_id & "','19000101000000',"
                            ssql = ssql & "DBO.FN_GETOPTIONCODE(N'EMPLOYEE_NAME',N'OPTION_TITLE',N'" & GVUpload.Rows(i).Cells(4).Text.Trim.ToUpper.Replace("'", "''") & "'),"
                            ssql = ssql & "N'" & GVUpload.Rows(i).Cells(3).Text.Trim.ToUpper.Replace("'", "''") & "'"
                            mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                            ssql = Nothing


                        End If
                        GVUpload.Rows(i).Cells(0).Text = "Sucessfully Insert."
                    End If
                    myDS2 = Nothing
                    ssql1 = Nothing
                    myds1 = Nothing
            ...
            End If
            'GVUpload.DataBind()
            'lblresult3.Text = "Import Sucessful."
        Catch exc As Exception
            lblresult3.Text = "Failed Insert Data"
        End Try
    End Sub
End Class