...
Partial Class PAGES_INFORMATION_SYSTEM_IMPORT_EMPLOYEE_RESIGN
    ...
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                ...
                Dim myData As New OleDbDataAdapter("SELECT * FROM [Emp Resignation$]", strConn)
        ...
        Try
            If GVUpload.Rows.Count > 0 Then
                For Me.i = 0 To GVUpload.Rows.Count - 1
                    ssql1 = "select ID from employee_profile where code = N'" & GVUpload.Rows(i).Cells(2).Text.Trim.ToUpper & "'"
                    ...
                    If myds1.Tables(0).Rows.Count > 0 Then
                        If CInt(myds1.Tables(0).Rows(0).Item(0)) > 0 Then
                            ...
                            If CInt(employee_id) > 0 Then
                                ssql2 = "select count(*) from employee_resign where employee_profile_id = N'" & employee_id & "'"
                                myDS2 = mySQL.ExecuteSQL(ssql2)
                                If CInt(myDS2.Tables(0).Rows(0).Item(0)) > 0 Then
                                    GVUpload.Rows(i).Cells(0).Text = "Employee Code Already Been Use."
                                Else
                                    ssql = "insert into employee_Resign(Company_Profile_Code,Employee_Profile_ID,Effective_Date,Join_Date,Confirm_Date,Resign_Date,Option_Resign_Flag,Submit_Date,Datetime_Create,User_Profile_Code_Create,Datetime_Modify,User_Profile_Code_Modify)"
                                    ssql = ssql & " Select N'" & GVUpload.Rows(i).Cells(1).Text.Trim.ToUpper.Replace("'", "''") & "',N'" & employee_id & "',dbo.fn_getcurrentdatetime(getdate()),"
                                    ssql = ssql & "isnull(N'" & mySetting.UnDisplayDateTime(Trim(GVUpload.Rows(i).Cells(3).Text.Trim.ToUpper.Replace("'", "''")), Session("Company").ToString, Session("Module").ToString) & "','19000101000000'),"
                                    ssql = ssql & "isnull(N'" & mySetting.UnDisplayDateTime(Trim(GVUpload.Rows(i).Cells(4).Text.Trim.ToUpper.Replace("'", "''")), Session("Company").ToString, Session("Module").ToString) & "','19000101000000'),"
                                    ssql = ssql & "isnull(N'" & mySetting.UnDisplayDateTime(Trim(GVUpload.Rows(i).Cells(5).Text.Trim.ToUpper.Replace("'", "''")), Session("Company").ToString, Session("Module").ToString) & "','19000101000000'),"
                                    ssql = ssql & "DBO.FN_GETOPTIONCODE(N'EMPLOYEE_RESIGN',N'Option_Resign_Flag',N'" & GVUpload.Rows(i).Cells(6).Text.Trim.ToUpper.Replace("'", "''") & "'),N'19000101000000',dbo.fn_getcurrentdatetime(getdate()),"
                    ...
                    myds1 = Nothing
                Next
                ssql = "Update employee_Resign set resign_date = '' where resign_date = 'P;BS&N00'"
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                ssql = Nothing
            End If
            'GVUpload.DataBind()
