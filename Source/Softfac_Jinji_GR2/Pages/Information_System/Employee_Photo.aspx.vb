    ...
    Private Sub BindGrid()
        Try
            If Trim(txtSEARCH_EMPLOYEE_CODE.Text) <> "Employee Code" And Trim(txtSEARCH_EMPLOYEE_NAME.Text) <> "Employee Name" Then
                ssql = "Exec sp_Generate_Query_Filter_WithSecurity '" & Session("Company") & "','" & Session("EmpID") & "','" & Session("Module") & "','Employee_Photo_Path_Vw@@Employee_Photo_Path_Vw','CODE@@NAME','" & Trim(txtSEARCH_EMPLOYEE_CODE.Text) & "@@" & Trim(txtSEARCH_EMPLOYEE_NAME.Text) & "','" & myGridView1.PageSize & "','" & Session("currentpage1") & "'"
        ...
        If e.CommandName.ToString.ToUpper = "UPLOAD" Then
            If ValidateUploadFile() = True Then
                Try
                    ...
                    Dim strUserID As String = mySetting.GetEmployeeCodeByCodeName_ReturnString(Session("Company").ToString, myGridView.Rows(intRow).Cells(2).Text.ToString.Replace("&amp;", "&"))
                    ...
                    Dim strEmpIDForUploadFile As String = mySetting.GetEmployeeIDbyCodeName_ReturnString(Session("Company").ToString, myGridView.Rows(intRow).Cells(2).Text.ToString.Replace("&amp;", "&"))
                    ...
                    objFindValues(1) = myGridView.Rows(intRow).Cells(2).Text.ToString.Replace("&amp;", "&")
                    objFindValues(2) = myGridView.Rows(intRow).Cells(3).Text.ToString.Replace("&amp;", "&")
