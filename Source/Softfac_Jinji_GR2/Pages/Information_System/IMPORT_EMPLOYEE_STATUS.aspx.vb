...
Partial Class PAGES_INFORMATION_SYSTEM_IMPORT_EMPLOYEE_STATUS
    ...
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                ...
                Dim myData As New OleDbDataAdapter("SELECT * FROM [Emp Status$]", strConn)
    ...
    Protected Sub imgBtnUpdateImport_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdateImport.Click
        Try
            If GVUpload.Rows.Count > 0 Then
                ssql = "delete from Import_employee_Status"
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                ssql = Nothing
                For Me.i = 0 To GVUpload.Rows.Count - 1
                    ssql1 = "select ID from employee_profile where code = N'" & GVUpload.Rows(i).Cells(2).Text.Trim.ToUpper.Replace("'", "''") & "'"
                    ...
                    If myds1.Tables(0).Rows.Count > 0 Then
                        If CInt(myds1.Tables(0).Rows(0).Item(0)) > 0 Then
                            ...
                            If CInt(employee_id) > 0 Then
                                ssql = "insert into import_employee_Status"
                                ssql = ssql & " Select N'" & GVUpload.Rows(i).Cells(1).Text.Trim.ToUpper.Replace("'", "''") & "',N'" & employee_id & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(3).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(4).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(5).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(6).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(7).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(8).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(9).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(10).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(11).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ssql = ssql & "N'" & GVUpload.Rows(i).Cells(12).Text.Trim.ToUpper.Replace("'", "''") & "',"
                    ...
                    myds1 = Nothing
                Next
                ssql = "exec SP_PR_IMPORTEMPSTATUS"
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                ssql = Nothing
            End If
            'GVUpload.DataBind()
