...
Partial Class Pages_InformationSystem_Employee_Personal
...
#Region "Public Declaration"
    ...
    Private WithEvents myUpdate As DataSet 'change for performance when update --sytey      
...
#Region "Private Declaration"
    ...
    Dim ssql2 As String, ssql3 As String, ssql4 As String, ssql5 As String, ssql6 As String, ssql7 As String, ssql8 As String
    Dim rowPosition As String = "R_", rowPositionM As String = "RM_", panelPosition As String = "Display_"
    Dim buttonPosition1 As String = "B1_", buttonPosition2 As String = "B2_", buttonPosition3 As String = "B3_", ButtonPosition4 As String = "B4_", ButtonPosition5 As String = "B5_", labelPosition1 As String = "L1_"
    Dim btnColourDef, btnColourAlt As String
    Dim strPath As String = "../../Images"
    Dim logic As Boolean
    Dim strEmpCnt As String
...
#Region "Page Setting"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            PagePreload()
            imgBtnFilter_Click(Nothing, Nothing)
            'BindGrid()
    ...
    Sub PagePreload()
        ...
        mySetting.GetBtnImgUrl(imgBtnRefresh, Session("Company").ToString, btnColourDef, "btnClear.png")
        ...
        mySetting.GetBtnImgUrl(ImgbtnAdd1, Session("Company").ToString, btnColourDef, "btnAdd.png")
        mySetting.GetBtnImgUrl(imgBtnFilter, Session("Company").ToString, btnColourDef, "btnFilter.png")
        mySetting.GetBtnImgUrl(ImgBtnQuery, Session("Company").ToString, btnColourDef, "btnQuery.png")
        ...
        mySetting.GetBtnImgUrl(imgbtnPrevioPage, Session("Company").ToString, btnColourDef, "btnPrevioPage.png")
        mySetting.GetBtnImgUrl(imgbtnNextPage, Session("Company").ToString, btnColourDef, "btnNextPage.png")
        ...
        'get Page Title
        ssql = "exec sp_sa_GetPageTitle '" & "Employee_Personal_VW" & "'"
        myDS = New DataSet
        ...
        myDS = New DataSet
        'myDS = mySetting.GetLabelDescription(Form.ID)
        ...
        If myDS.Tables.Count > 1 Then
            ...
            If myDT1.Rows.Count > 0 Then
                ...
                ImgbtnAdd1.Visible = AllowInsert
                ...
                imgBtnRefresh.Visible = AllowView
                imgBtnCancel.Visible = AllowView
                imgBtnPrint.Visible = AllowPrint
                ImgBtnQuery.Visible = AllowPrint
            End If
    ...
    Sub BindGrid()
        Try
            ...
            If CType(Session("SearchByFilter"), Boolean) = True Then
                ssql = Session("SearchSQL").ToString.Trim & _currentPageNumber.ToString.Trim & "'"
...
#Region "Panel Edit"
    Private Function ValidateInsert3() As Boolean
        lblresult2.Visible = True
        If txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim <> "" Then
            If txtEMPLOYEE_PROFILE_ID.Text.IndexOf("[", 1) > 0 Then
                lblresult2.Text = "Cannot use '[' character for the Employee Code!"
    ...
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        If (txtEMPLOYEE_PROFILE_ID.Text = "") Then
            txtEMPLOYEE_PROFILE_ID.Text = auto_Generate_EmpID(False)
            strFlag = True
        End If

        If ValidateInsert() = True And ValidateInsert2() = True And ValidateInsert3() = True Then
            ...
            If myDS.Tables(0).Rows.Count > 0 Then
                ...
                Exit Sub
            Else
                Dim tmpssql As String = "Select ID From Employee_Profile Where Company_Profile_Code=dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text.ToString.Trim & "') And Code='" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "'"
                myDS = New DataSet
                myDS = mySQL.ExecuteSQL(ssql & "; " & tmpssql, Session("Company").ToString, Session("EmpID").ToString)
                If Not myDS Is Nothing Then
                    Session("MaxEmpID") = myDS.Tables(0).Rows(0).Item(0).ToString
                    If ValidateInsert2() = False Then
                        mySQL.ExecuteSQL("Delete From Employee_Profile Where ID=" & Session("MaxEmpID").ToString, Session("Company").ToString, Session("EmpID").ToString)
                        Exit Sub
                    End If
                End If
            End If
            ...
            ssql2 = ssql4
            ssql3 = "Insert Into Employee_Resign(Company_Profile_Code,Employee_Profile_ID,Effective_Date,Join_Date,Confirm_Date,Resign_Date," & _
                                    "Submit_Date,Option_Resign_Type,Option_Resign_Flag,Remark,DateTime_Create,User_Profile_Code_Create,DateTime_Modify,User_Profile_Code_Modify) Values('" & Session("Company").ToString & "','" & Session("MaxEmpID").ToString & _
                                    "',dbo.fn_GetCurrentDate(GetDate()),dbo.fn_GetCurrentDate(GetDate()),'19000101000000'," & _
                                    "'19000101000000','19000101000000','','ACTIVE','',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "')"

            myDS = New DataSet
            myDS = mySQL.ExecuteSQL("Select Option_Type,Code,ID From Organisation_Code_Profile_Vw Where Company_Profile_Code='" & Session("Company").ToString & "' And Code='~~NONE'", Session("Company").ToString, Session("EmpID").ToString)
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                htb.Add(myDS.Tables(0).Rows(i).Item(0).ToString, myDS.Tables(0).Rows(i).Item(2).ToString)
            Next

            ssql4 = "Insert Into Employee_Status(Company_Profile_Code,Employee_Profile_ID,OCP_ID_Division,OCP_ID_Department," & _
                    "OCP_ID_Section,OCP_ID_Line,OCP_ID_Group,OCP_ID_Process,OCP_ID_Finance,OCP_ID_TMS," & _
                    "OCP_ID_Job_Grade,OCP_ID_Job_Title,OCP_ID_Appraisal,Option_Employee_Group,Option_Employment_Type,DateTime_Create,User_Profile_Code_Create,DateTime_Modify,User_Profile_Code_Modify) Values('" & Session("Company").ToString & "','" & _
                    Session("MaxEmpID").ToString & "','" & GetOCPValue("DIVISION") & "','" & GetOCPValue("DEPARTMENT") & _
                    "','" & GetOCPValue("SECTION") & "','" & GetOCPValue("LINE") & "','" & GetOCPValue("GROUP") & _
                    "','" & GetOCPValue("PROCESS") & "','" & GetOCPValue("FINANCE") & "','" & GetOCPValue("TMS") & _
                    "','" & GetOCPValue("JOB_GRADE") & "','" & GetOCPValue("JOB_TITLE") & _
                    "','" & GetOCPValue("APPRAISAL") & "','','',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "')"

            ssql5 = "Insert Into Employee_Salary(Employee_Profile_ID,Basic_Salary,Currency_Profile_Code,Option_Pay_Mode," & _
                    "Option_Pay_Type,Option_Pay_Cycle,DateTime_Create,User_Profile_Code_Create,DateTime_Modify,User_Profile_Code_Modify) Values('" & Session("MaxEmpID").ToString & "',0,'" & GetCompanyProfileCode() & "','BANK','MONTHLY','MONTHLY',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "')"
            ssql6 = "Insert Into Employee_Increment (Company_Profile_Code,Employee_Profile_ID,Effective_Date,Option_Type_Increment," & _
                    "Old_Salary,New_Salary,Increment,Promotion,OCP_ID_Job_Grade_Old,OCP_ID_Job_Grade_New," & _
                    "OCP_ID_Job_Title_Old,OCP_ID_Job_Title_New,OCP_ID_Appraisal_Old,OCP_ID_Appraisal_New,DateTime_Create,User_Profile_Code_Create,DateTime_Modify,User_Profile_Code_Modify) " & _
                    "Values('" & Session("Company").ToString & "','" & Session("MaxEmpID").ToString & "',dbo.fn_GetCurrentDate(GetDate()),'NEW_JOIN',0,0,0,0,'" & _
                    GetOCPValue("JOB_GRADE") & "','" & GetOCPValue("JOB_GRADE") & "','" & GetOCPValue("JOB_TITLE") & _
                    "','" & GetOCPValue("JOB_TITLE") & "','" & GetOCPValue("APPRAISAL") & "','" & _
                    GetOCPValue("APPRAISAL") & "',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "',dbo.fn_GetCurrentDate(GetDate()),'" & Session("EmpID").ToString & "')"
            ssql7 = "Insert Into Employee_Category(Company_Profile_Code,Employee_Profile_ID,OCP_ID_Category01,OCP_ID_Category02,OCP_ID_Category03," & _
                    "OCP_ID_Category04,OCP_ID_Category05,OCP_ID_Category06,OCP_ID_Category07,OCP_ID_Category08," & _
                    "OCP_ID_Category09,OCP_ID_Category10,Effective_Date) Values('" & Session("Company").ToString & "','" & Session("MaxEmpID").ToString & "','" & _
                    GetOCPValue("CATEGORY") & "','" & GetOCPValue("CATEGORY") & "','" & GetOCPValue("CATEGORY") & "','" & _
                    GetOCPValue("CATEGORY") & "','" & GetOCPValue("CATEGORY") & "','" & GetOCPValue("CATEGORY") & "','" & _
                    GetOCPValue("CATEGORY") & "','" & GetOCPValue("CATEGORY") & "','" & GetOCPValue("CATEGORY") & "','" & _
                    GetOCPValue("CATEGORY") & "',dbo.fn_GetCurrentDateTime(GetDate()))"
            ssql8 = "Insert Into Employee_No_Of_Child(Employee_Profile_ID,Work_Child,Non_Work_Child,Option_Spouse_Work_Status) Values(N'" & Session("MaxEmpID").ToString & "','0','0','NO')"
            Dim tmpHtb As New Hashtable
            tmpHtb.Add("ssql1", ssql2)
            tmpHtb.Add("ssql2", ssql3)
            tmpHtb.Add("ssql3", ssql4)
            tmpHtb.Add("ssql4", ssql5)
            tmpHtb.Add("ssql5", ssql6)
            tmpHtb.Add("ssql6", ssql7)
            tmpHtb.Add("ssql7", ssql8)
            Try
                mySQL.ExecuteSQLTransactionByHashtable(tmpHtb, "Employee_Personal_Vw", Session("Company").ToString, Session("EmpID").ToString)
                If strFlag = True Then
                    txtEMPLOYEE_PROFILE_ID.Text = ""
                    EmpID = auto_Generate_EmpID(True)
                End If
            ...
            'mySQL.ExecuteSQL7Transaction(ssql2, ssql3, ssql4, ssql5, ssql6, ssql7, ssql8, "Employee_Personal_Vw")
            'mySQL.ExecuteSQL3Transaction("Set Identity_Insert On;" & ssql & ";Set Identity_Insert Off;", ssql4, ssql6, "Employee_Personal_Vw")

            Dim message = "Auto Generate New Employee ID is " + EmpID + " ."
        ...
        BindGrid()
    End Sub
    Private Function GetCompanyProfileCode() As String
        Dim tmpds As New DataSet
        tmpds = mySQL.ExecuteSQL("Select Currency_Profile_Code From Company_Profile Where Code='" & Session("Company").ToString & "'", Session("Company").ToString, Session("EmpID").ToString)
        ...
        End If
    End Function
    Private Function GetOCPValue(ByVal strType As String) As String
        Dim de As DictionaryEntry, RecFound As Boolean = False

        For Each de In htb
            If de.Key = strType Then
                RecFound = True
                Exit For
            End If
        Next
...
#Region "Panel GridView"
    ...
    Private Function myGridViewValidateUpdate(ByVal RowIndex As Integer) As Boolean
        lblresult2.Visible = True
        myDS = mySQL.ExecuteSQL("Select Code,Name,Option_Data_Type,Option_Mandatory From Table_Field Where Table_Profile_Code='" & Form.ID & "' And Option_Primary_Key='NO' And Option_View='YES' Order By Table_Profile_Code,Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
        ssql = "Update " & Form.ID & " Set "
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                For Me.j = 2 To myGridView.HeaderRow.Cells.Count - 1
                            ...
                            Select Case myDS.Tables(0).Rows(i).Item(2).ToString
                                Case "DATE", "DATETIME"
                                    If IsNumeric(mySetting.ConvertDateToDecimal(myTextBox.Text, Session("Company").ToString, Session("Module").ToString)) Then
                                        ...
                                        myDST = mySQL.ExecuteSQL("Select Function_Name,Default_Value From User_Define_Function Where Company_Profile_Code='" & Session("Company").ToString & "' And Module_Profile_Code='" & Session("Module").ToString & "' And Table_Profile_Code='" & Form.ID & "' And Table_Field_Code='" & myDS.Tables(0).Rows(i).Item(0).ToString & "' And Query_Action='UPDATE'", Session("Company").ToString, Session("EmpID").ToString)
            ...
            myDS2 = mySQL.ExecuteSQL("Select Code,Name,Option_Data_Type,Option_Mandatory From Table_Field Where Table_Profile_Code='" & Form.ID & "' And Option_Primary_Key='YES' And Option_View='YES' Order By Table_Profile_Code,Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
...
#Region "Panel Action"
    ...
    Protected Sub imgBtnAdd_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnAdd.Click
        ...
        lblresult2.Visible = True
        EnableAll()
        imgKeyOption_Resign_Flag.Visible = False
        lblOption_Resign_Flag.Visible = False
        ddlOption_Resign_Flag.Visible = False
        imgbtnOption_Resign_Flag.Visible = False
        ...
        imgBtnRefresh.CssClass = buttonPosition2
        ...
        If ActionValidateEdit() Then
            ...
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                If chkEdit.Checked Then
                    chkEdit.Checked = False
                    myDS = New DataSet
                    myDS = mySQL.ExecuteSQL("SELECT Code,Option_Data_Type,Name FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' ORDER BY SEQUENCE_NO", Session("Company").ToString, Session("EmpID").ToString)
                    For Me.j = 0 To myDS.Tables(0).Rows.Count - 1
                        For Me.intColumn = 1 To gvHistory.HeaderRow.Cells.Count - 1
        ...
        If ActionValidateDelete() Then
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                If chkDelete.Checked Then
                    ssql3 = GetSQLParameter("Employee_Profile", gvHistory, i, Session("Company").ToString, Session("Module").ToString)
                    ssql4 = GetSQLParameter2("Employee_Name", gvHistory, i, Session("Company").ToString, Session("Module").ToString)

                    If ssql3 <> Nothing And ssql4 <> Nothing Then
                        Dim tmpEmpID As String = GetEmpID(i)
                        Dim chkSSQL As String = "Select Count(*) As Count1 From Employee_Leave_Application Where Employee_Profile_ID='" & Session("EmpIDToDelete").ToString & "';" & _
                                                "Select Count(*) As Count2 From Employee_Attendance Where Employee_Profile_ID='" & Session("EmpIDToDelete").ToString & "'"
                        myDS = New DataSet
                        myDS = mySQL.ExecuteSQL(chkSSQL, Session("Company").ToString, Session("EmpID").ToString)
                        If CInt(myDS.Tables(0).Rows(0).Item(0).ToString) > 0 Or CInt(myDS.Tables(1).Rows(0).Item(0).ToString) > 0 Then
                            lblresult.Visible = True
                            lblresult.Text = "Data Being Used. Employee cannot be delete!"
                            Exit For
                        Else
                            lblresult2.Visible = False
                        End If
                        ...
                        'ssql = ssql3
                        ssql = "Delete From Employee_Profile Where Company_Profile_Code='" & _
                                Session("Company").ToString & "' And [ID]=" & tmpEmpID
                        ssql2 = ssql4
                        ssql2 = "Delete From Employee_Name Where Employee_Profile_ID=" & tmpEmpID
                        ssql3 = "Delete From Employee_Resign Where Company_Profile_Code='" & _
                                Session("Company").ToString & "' And Employee_Profile_ID=" & tmpEmpID
                        ssql4 = "Delete From Employee_Status Where Company_Profile_Code='" & _
                                Session("Company").ToString & "' And Employee_Profile_ID=" & tmpEmpID
                        ssql5 = "Delete From Employee_Salary Where Employee_Profile_ID=" & tmpEmpID
                        ssql6 = "Delete From Employee_Increment Where Company_Profile_Code='" & _
                                Session("Company").ToString & "' And Employee_Profile_ID=" & tmpEmpID
                        ssql7 = "Delete From Employee_Category Where Company_Profile_Code='" & _
                                Session("Company").ToString & "' And Employee_Profile_ID=" & tmpEmpID
                        ssql8 = "Delete From Employee_No_Of_Child Where Employee_Profile_ID=" & tmpEmpID

                        mySQL.ExecuteSQL8Transaction(ssql8, ssql2, ssql3, ssql4, ssql5, ssql6, ssql7, ssql, "Delete Employee_Personal_Vw")
                        tmpEmpID = ""
                        ssql = Nothing
                    End If
    ...
    Private Function GetEmpID(ByVal RowIndex As Integer) As String
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select Name From Table_Field Where Table_Profile_Code='" & Form.ID & "' And Code='Employee_Profile_ID'", Session("Company").ToString, Session("EmpID").ToString)
        If Not myDS Is Nothing Then
            For Me.j = 0 To myGridView.HeaderRow.Cells.Count - 1
                If myDS.Tables(0).Rows(0).Item(0).ToString = myGridView.HeaderRow.Cells(j).Text.ToString Then
                    Dim tmpDs As New DataSet, sqlString As String = ""
                    sqlString = "Select Isnull(dbo.fn_ReturnEmpIDByCodeName('" & Session("Company").ToString & "','" & myGridView.Rows(RowIndex).Cells(j).Text.ToString.Replace("&amp;", "&").Replace("&nbsp;", "&").Replace("'", "''") & "'),'') As Employee_Profile_ID"
                    tmpDs = mySQL.ExecuteSQL(sqlString, Session("Company").ToString, Session("EmpID").ToString)
                    If Not tmpDs Is Nothing Then
                        Session("EmpIDToDelete") = tmpDs.Tables(0).Rows(0).Item(0).ToString
                    Else
                        Session("EmpIDToDelete") = "0"
                    End If
                    tmpDs = Nothing
                    Exit For
                End If
            Next
        End If
        Return Session("EmpIDToDelete").ToString
    End Function
    Private Function GetSQLParameter(ByVal FormID As String, ByVal myGridView As System.Web.UI.WebControls.GridView, ByVal RowIndex As Integer, ByVal strCompanyID As String, ByVal strModule As String) As String
        Dim i As Integer = 0
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select a.Code,a.Name,a.Option_Data_Type,a.Option_Default_Value From Table_Field a Left Outer Join Table_Field b On a.Code=b.Code Where a.Table_Profile_Code='Employee_Personal_Vw' And b.Table_Profile_Code='" & FormID & "' And a.Option_View_Card='YES' And a.Option_Primary_Key='YES' Order By a.Table_Profile_Code,a.Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                For Me.j = 1 To myGridView.Rows(RowIndex).Cells.Count - 1
                    If myDS.Tables(0).Rows(i).Item(1).ToString = myGridView.HeaderRow.Cells(j).Text.ToString Then
                        Select Case myDS.Tables(0).Rows(i).Item(2).ToString
                            Case "DATE"
                                ssql = ssql & myDS.Tables(0).Rows(i).Item(0).ToString & "='" & mySetting.ConvertDateToDecimal(myGridView.Rows(RowIndex).Cells(j).Text.ToString, strCompanyID, strModule) & "' And "
                            Case "DATETIME"
                                ssql = ssql & myDS.Tables(0).Rows(i).Item(0).ToString & "='" & mySetting.UnDisplayDateTime(myGridView.Rows(RowIndex).Cells(j).Text.ToString, strCompanyID, strModule) & "' And "
                            Case "TIME"
                                ssql = ssql & myDS.Tables(0).Rows(i).Item(0).ToString & "='" & mySetting.UnDisplayTime(myGridView.Rows(RowIndex).Cells(j).Text.ToString, strCompanyID, strModule) & "' And "
                            Case "OPTION"
                            ...
                            Case Else
                                myDS2 = New DataSet
                                myDS2 = mySQL.ExecuteSQL("Select Function_Name From User_Define_Function Where Company_Profile_Code='" & strCompanyID & "' And Module_Profile_Code='" & strModule & "' And Table_Profile_Code='" & "Employee_Personal_Vw" & "' And Table_Field_Code='" & myDS.Tables(0).Rows(i).Item(0) & "' And Query_Action='DELETE'", Session("Company").ToString, Session("EmpID").ToString)
                                If myDS2.Tables(0).Rows.Count > 0 Then
                                    ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & "=" & myDS2.Tables(0).Rows(0).Item(0).ToString & "('" & myGridView.Rows(RowIndex).Cells(j).Text.ToString & "') And "
                                Else
                                    ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & "='" & myGridView.Rows(RowIndex).Cells(j).Text.ToString & "' And "
                                End If
                                myDS2 = Nothing
                        End Select
                    End If
                Next
            Next
            If ssql.IndexOf("And CODE=") = -1 Then
                Dim tmpDS As New DataSet, strDisplayName As String = ""
                tmpDS = mySQL.ExecuteSQL("SELECT Name FROM Table_Field WHERE Table_Profile_Code='" & Form.ID.ToString & "' And Code='Employee_Profile_ID'", Session("Company").ToString, Session("EmpID").ToString)
                If Not tmpDS Is Nothing Then
                    If tmpDS.Tables.Count > 0 Then
                        If tmpDS.Tables(0).Rows.Count > 0 Then
                            strDisplayName = tmpDS.Tables(0).Rows(0).Item(0).ToString
                        End If
                    End If
                End If
                tmpDS = Nothing
                ...
                For Me.j = 1 To myGridView.Rows(RowIndex).Cells.Count - 1
                    If strDisplayName = myGridView.HeaderRow.Cells(j).Text.ToString Then
                        ssql = ssql & "CODE=dbo.fn_ReturnEmpCodeByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(RowIndex).Cells(j).Text.ToString & "') And "
                        Exit For
                    End If
                Next
            End If
        ...
        Return ssql
    End Function
    Private Function GetSQLParameter2(ByVal FormID As String, ByVal myGridView As System.Web.UI.WebControls.GridView, ByVal RowIndex As Integer, ByVal strCompanyID As String, ByVal strModule As String) As String
        Dim i As Integer = 0
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select a.Code,a.Name,a.Option_Data_Type,a.Option_Default_Value From Table_Field a Left Outer Join Table_Field b On a.Code=b.Code Where a.Table_Profile_Code='Employee_Personal_Vw' And b.Table_Profile_Code='" & FormID & "' And a.Option_View_Card='YES' And a.Option_Primary_Key='YES' Order By a.Table_Profile_Code,a.Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
        'myDS = mySQL.ExecuteSQL("Select Code,Name,Option_Data_Type,Option_Default_Value From Table_Field Where Table_Profile_Code='" & FormID & "' And Option_Primary_Key='YES' Order By Table_Profile_Code,Sequence_No")
                        ...
                        Select Case myDS.Tables(0).Rows(i).Item(2).ToString
                            ...
                            Case Else
                                If myDS.Tables(0).Rows(i).Item(0).ToString.ToUpper = "EMPLOYEE_PROFILE_ID" Then
                                    ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & "=dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(RowIndex).Cells(j).Text.ToString & "') And "
                                Else
                                    myDS2 = New DataSet
                                    myDS2 = mySQL.ExecuteSQL("Select Function_Name From User_Define_Function Where Company_Profile_Code='" & strCompanyID & "' And Module_Profile_Code='" & strModule & "' And Table_Profile_Code='" & "Employee_Personal_Vw" & "' And Table_Field_Code='" & myDS.Tables(0).Rows(i).Item(0) & "' And Query_Action='DELETE'", Session("Company").ToString, Session("EmpID").ToString)
                                    If myDS2.Tables(0).Rows.Count > 0 Then
    ...
    Protected Sub imgBtnFilter_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnFilter.Click
        ...
        ImgbtnAdd1.CssClass = ButtonPosition4
        imgBtnCancel.CssClass = buttonPosition3
        imgBtnRefresh.CssClass = buttonPosition2
...
#Region "Sub & Function"
    ...
    Private Function ActionValidateUpdate() As Boolean
        ...
        End If
    End Function
    Private Sub AutoAdjustPosition(ByVal strSelect As String)
        Try
            ...
            myDS = New DataSet
            myDS = mySQL.ExecuteSQL("SELECT Code,Option_Data_Type,Option_View_Card,Option_Editable FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' ORDER BY SEQUENCE_NO", Session("Company").ToString, Session("EmpID").ToString)
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                Dim myLabel As Label = Page.FindControl("lbl" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
                Dim myImgBtn As ImageButton = Page.FindControl("imgbtn" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
                Dim myImgKey As ImageButton = Page.FindControl("imgKey" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
                If myDS.Tables(0).Rows(i).Item(2).ToString = "YES" Then
                    ...
                    Select Case myDS.Tables(0).Rows(i).Item(1).ToString.Trim
                        Case "OPTION"
                            ...
                            If myLabel.Visible = True And myDDL.Visible = True Then
                                autonum += 1
                                myImgKey.CssClass = myPosition & autonum
                                autonum += 1
                                myLabel.CssClass = myPosition & autonum
                                autonum = autonum + 1
                                myDDL.CssClass = myPosition & autonum
                                autonum = autonum + 1
                                myImgBtn.CssClass = myPosition & autonum
                            End If
                            If myDS.Tables(0).Rows(i).Item(3).ToString = "NO" And strSelect = "3" Then
                                myImgBtn.Visible = False
                                myDDL.Enabled = False
                                myLabel.Enabled = False
                            End If
                            myDDL = Nothing
                        Case "DATE", "DATETIME", "TIME", "LOOKUP"
                            Dim myTextBox As TextBox = Page.FindControl("txt" & myDS.Tables(0).Rows(i).Item(0).ToString.Trim)
            ...
            ButtonPosition4 = ButtonPosition4 & autonum - 1
            ButtonPosition5 = ButtonPosition5 & autonum - 1
    ...
    Private Sub CheckAll()
        For Me.i = 0 To myGridView.Rows.Count - 1
            ...
            chkDelete = Nothing
        Next
        For Me.i = 0 To gvHistory.Rows.Count - 1
    ...
    Private Sub EnableAll()
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("SELECT Code,Option_Primary_Key,Option_Data_Type,Option_Mandatory FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' AND Option_View_Card='YES' ORDER BY SEQUENCE_NO", Session("Company").ToString, Session("EmpID").ToString)
        For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
    ...
    Private Sub LoadKeyImage()
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("SELECT Code,Option_Mandatory FROM Table_Field Where Table_Profile_Code='" & Form.ID & "' ORDER BY SEQUENCE_NO", Session("Company").ToString, Session("EmpID").ToString)
        For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
    ...
    Private Function ValidateInsert() As Boolean
        lblresult2.Visible = True
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select Isnull(Max(ID),0) + 1 From Employee_Profile", Session("Company").ToString, Session("EmpID").ToString)
        Session("MaxEmpID") = myDS.Tables(0).Rows(0).Item(0)
        myDS = Nothing
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select a.Code,a.Option_Data_Type,a.Option_Editable,a.Option_Mandatory,a.Option_Default_Value,a.Name,a.Option_Primary_Key From Table_Field a Left Outer Join Table_Field b On a.Code=b.Code Where a.Table_Profile_Code='Employee_Personal_Vw' And b.Table_Profile_Code='Employee_Profile' And a.Option_View_Card='YES' Order By a.Table_Profile_Code,a.Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
        ssql = "Insert Into " & "Employee_Profile(" '& "(ID,"
        ssql2 = ") Values(" '& Session("MaxEmpID").ToString & ","
        ssql3 = "Select * From " & "Employee_Profile" & " Where "
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                If IsNumeric(mySetting.ConvertDateToDecimal(myTextBox.Text, Session("Company").ToString, Session("Module").ToString)) = True Then
                                    ...
                                    Dim myDST As New DataSet
                                    myDST = mySQL.ExecuteSQL("Select Function_Name,Default_Value From User_Define_Function Where Company_Profile_Code='" & Session("Company").ToString & "' And Module_Profile_Code='" & Session("Module").ToString & "' And Table_Profile_Code='" & "Employee_Personal_Vw" & "' And Table_Field_Code='" & myDS.Tables(0).Rows(i).Item(0).ToString & "' And Query_Action='INSERT'", Session("Company").ToString, Session("EmpID").ToString)
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        If myDST.Tables(0).Rows(0).Item(1).ToString.Trim = "" Then
                                ...
                                Else
                                    If myDS.Tables(0).Rows(i).Item(3).ToString = "YES" Then
                                        lblresult2.Text = "Invalid Input Date Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                                        myTextBox.Focus()
                                        Return False
                                    Else
                                        ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & ","
                    ...
                    Case "DATETIME"
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ...
                                Else
                                    If myDS.Tables(0).Rows(i).Item(3).ToString = "YES" Then
                                        lblresult2.Text = "Invalid Input DateTime Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                                        myTextBox.Focus()
                                        Return False
                                    Else
                                        ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & ","
                    ...
                    Case "TIME"
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ...
                                Else
                                    If myDS.Tables(0).Rows(i).Item(3).ToString = "YES" Then
                                        lblresult2.Text = "Invalid Input Time Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                                        myTextBox.Focus()
                                        Return False
                                    Else
                                        ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & ","
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                If IsNumeric(myTextBox.Text) = True Then
                                    ...
                                    End If
                                Else
                                    lblresult2.Text = "Invalid Input Numeric Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                                    myTextBox.Focus()
                                    Return False
                                End If
                            Else
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ...
                                If myDST.Tables(0).Rows.Count > 0 Then
                                    ...
                                    Else
                                        ssql2 = ssql2 & "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(" & myDST.Tables(0).Rows(0).Item(1).ToString.Trim & "),''),"
                                    End If
            ...
            DefaultDS = mySQL.ExecuteSQL("SELECT Code,Option_Data_Type,Default_Value_Date,Default_Value_DateTime,Default_Value_Time,Default_Value_Integer,Default_Value_Decimal,Default_Value_Character,Option_Primary_Key FROM Table_Field Where Table_Profile_Code='" & "Employee_Profile" & "' AND Option_View_Card='NO' AND Option_Default_Value='YES' ORDER BY Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
            If Not DefaultDS Is Nothing Then
                If DefaultDS.Tables.Count > 0 Then
                    If DefaultDS.Tables(0).Rows.Count > 0 Then
                        For Me.i = 0 To DefaultDS.Tables(0).Rows.Count - 1
            ...
            If txtEMPLOYEE_PROFILE_ID.Visible = True Then
                ssql = ssql & "CODE,"
                ssql2 = ssql2 & "'" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "',"
                ssql3 = ssql3 & "CODE='" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "' And "
            End If
    ...
    End Function
    Private Function ValidateInsert2() As Boolean
        lblresult2.Visible = True
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select a.Code,a.Option_Data_Type,a.Option_Editable,a.Option_Mandatory,a.Option_Default_Value,a.Name,a.Option_Primary_Key From Table_Field a Left Outer Join Table_Field b On a.Code=b.Code Where a.Table_Profile_Code='Employee_Personal_Vw' And b.Table_Profile_Code='Employee_Name' And a.Option_View_Card='YES' Order By a.Table_Profile_Code,a.Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
        ssql4 = "Insert Into " & "Employee_Name" & "("
        ssql5 = ") Values("
        ssql6 = "Select * From " & "Employee_Name" & " Where "
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                Select Case myDS.Tables(0).Rows(i).Item(1).ToString
                    Case "DATE"
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                If IsNumeric(mySetting.ConvertDateToDecimal(myTextBox.Text, Session("Company").ToString, Session("Module").ToString)) = True Then
                                    ...
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        If myDST.Tables(0).Rows(0).Item(1).ToString.Trim = "" Then
                                            ssql5 = ssql5 & "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "'),'0'),"
                                        Else
                                            ssql5 = ssql5 & "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(" & myDST.Tables(0).Rows(0).Item(1).ToString.Trim & "),'0'),"
                                        End If
                                    Else
                                        ssql5 = ssql5 & "'" & mySetting.ConvertDateToDecimal(Trim(myTextBox.Text), Session("Company").ToString, Session("Module").ToString) & "',"
                                    End If
                                    myDST = Nothing
                                    If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                        ssql6 = ssql6 & myDS.Tables(0).Rows(i).Item(0) & "='" & mySetting.ConvertDateToDecimal(myTextBox.Text, Session("Company").ToString, Session("Module").ToString) & "' And "
                                    End If
                                Else
                                    If myDS.Tables(0).Rows(i).Item(3).ToString = "YES" Then
                                        lblresult2.Text = "Invalid Input Date Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                            ...
                            Else
                                ...
                                Else
                                    ssql4 = ssql4 & myDS.Tables(0).Rows(i).Item(0) & ","
                                    ssql5 = ssql5 & "'" & Trim(myTextBox.Text) & "',"
                                End If
                            End If
                        End If
                        myTextBox = Nothing
                            ...
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                If IsNumeric(mySetting.UnDisplayDateTime(myTextBox.Text, Session("Company").ToString, Session("Module").ToString)) = True Then
                                    ...
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        ...
                                        End If
                                    Else
                                        ssql5 = ssql5 & "'" & mySetting.UnDisplayDateTime(Trim(myTextBox.Text), Session("Company").ToString, Session("Module").ToString) & "',"
                                    End If
                                    myDST = Nothing
                                    If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                        ssql6 = ssql6 & myDS.Tables(0).Rows(i).Item(0) & "='" & mySetting.UnDisplayDateTime(myTextBox.Text, Session("Company").ToString, Session("Module").ToString) & "' And "
                                    End If
                                Else
                                    If myDS.Tables(0).Rows(i).Item(3).ToString = "YES" Then
                                        lblresult2.Text = "Invalid Input DateTime Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                            ...
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                If IsNumeric(mySetting.UnDisplayTime(myTextBox.Text.ToString, Session("Company").ToString, Session("Module").ToString)) = True Then
                                    ...
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        ...
                                        End If
                                    Else
                                        ssql5 = ssql5 & "'" & mySetting.UnDisplayTime(Trim(myTextBox.Text), Session("Company").ToString, Session("Module").ToString) & "',"
                                    End If
                                    myDST = Nothing
                                    If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                        ssql6 = ssql6 & myDS.Tables(0).Rows(i).Item(0) & "='" & mySetting.UnDisplayTime(myTextBox.Text, Session("Company").ToString, Session("Module").ToString) & "' And "
                                    End If
                                Else
                                    If myDS.Tables(0).Rows(i).Item(3).ToString = "YES" Then
                                        lblresult2.Text = "Invalid Input Time Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                    ...
                    Case "OPTION"
                        ...
                        If myDropdownlist.Visible = True Then
                            If mySetting.CheckTextNull(myDropdownlist.SelectedValue) = True Then
                                If mySetting.ValidateInput(myDropdownlist, myDS.Tables(0).Rows(i).Item(1).ToString) = True Then
                                    ssql4 = ssql4 & myDS.Tables(0).Rows(i).Item(0) & ","
                                    ssql5 = ssql5 & "'" & Trim(myDropdownlist.SelectedValue) & "',"
                                    If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                        ssql6 = ssql6 & myDS.Tables(0).Rows(i).Item(0) & "='" & Trim(myDropdownlist.SelectedValue) & "' And "
                                    End If
                                Else
                                    lblresult2.Text = "Invalid Selection For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                                    myDropdownlist.Focus()
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                If IsNumeric(myTextBox.Text) = True Then
                                    ...
                                    ssql5 = ssql5 & "'" & Trim(myTextBox.Text) & "',"
                                    If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                        ssql6 = ssql6 & myDS.Tables(0).Rows(i).Item(0) & "='" & Trim(myTextBox.Text) & "' And "
                                    End If
                                Else
                                    lblresult2.Text = "Invalid Input Numeric Format For " & Trim(myDS.Tables(0).Rows(i).Item(0)) & " !"
                                    myTextBox.Focus()
                    ...
                    Case Else   '"CHARACTER", "LOOKUP"
                        ...
                        If myTextBox.Visible = True Then
                            If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                ssql4 = ssql4 & myDS.Tables(0).Rows(i).Item(0) & ","
                                If myDS.Tables(0).Rows(i).Item(0).ToString = "EMPLOYEE_PROFILE_ID" Then
                                    ssql5 = ssql5 & "'" & Session("MaxEmpID").ToString & "',"
                                    If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                        ssql6 = ssql6 & myDS.Tables(0).Rows(i).Item(0).ToString & "='" & Session("MaxEmpID").ToString & "' And "
                                    End If
                                Else
                                    Dim myDST As New DataSet
                                    myDST = mySQL.ExecuteSQL("Select Function_Name,Default_Value From User_Define_Function Where Company_Profile_Code='" & Session("Company").ToString & "' And Module_Profile_Code='" & Session("Module").ToString & "' And Table_Profile_Code='" & "Employee_Personal_Vw" & "' And Table_Field_Code='" & myDS.Tables(0).Rows(i).Item(0).ToString & "' And Query_Action='INSERT'", Session("Company").ToString, Session("EmpID").ToString)
                                    If myDST.Tables(0).Rows.Count > 0 Then
                                        If myDST.Tables(0).Rows(0).Item(1).ToString.Trim = "" Then
                                            ssql5 = ssql5 & "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "'),''),"
                                        Else
                                            ssql5 = ssql5 & "ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(" & myDST.Tables(0).Rows(0).Item(1).ToString.Trim & "),''),"
                                        End If
                                        If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
                                            ssql6 = ssql6 & myDS.Tables(0).Rows(i).Item(0) & "=ISNULL(" & myDST.Tables(0).Rows(0).Item(0).ToString & "(N'" & myTextBox.Text.ToString & "'),0) And "
                                        End If
                                    Else
                                        ssql5 = ssql5 & "'" & Trim(myTextBox.Text) & "',"
                                        If myDS.Tables(0).Rows(i).Item(6).ToString = "YES" Then
            ...
            Dim DefaultDS As New DataSet
            DefaultDS = mySQL.ExecuteSQL("SELECT Code,Option_Data_Type,Default_Value_Date,Default_Value_DateTime,Default_Value_Time,Default_Value_Integer,Default_Value_Decimal,Default_Value_Character,Option_Primary_Key FROM Table_Field Where Table_Profile_Code='" & "Employee_Name" & "' AND Option_View_Card='NO' AND Option_Default_Value='YES' ORDER BY Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
            If Not DefaultDS Is Nothing Then
                If DefaultDS.Tables.Count > 0 Then
                    If DefaultDS.Tables(0).Rows.Count > 0 Then
                        For Me.i = 0 To DefaultDS.Tables(0).Rows.Count - 1
                            ...
                            Select Case DefaultDR(1).ToString
                                Case "DATE"
                                    If DefaultDR("Default_Value_Date").ToString.Trim <> "" Then
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'" & DefaultDR("Default_Value_Date").ToString.Trim & "',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='" & DefaultDR("Default_Value_Date").ToString.Trim & "' And "
                                        End If
                                    Else
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "dbo.fn_GetCurrentDate(GetDate()),"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "=dbo.fn_GetCurrentDate(GetDate()) And "
                                        End If
                                    End If
                                Case "DATETIME"
                                    If DefaultDR("Default_Value_DateTime").ToString.Trim <> "" Then
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'" & DefaultDR("Default_Value_DateTime").ToString.Trim & "',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='" & DefaultDR("Default_Value_DateTime").ToString.Trim & "' And "
                                        End If
                                    Else
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "dbo.fn_GetCurrentDateTime(GetDate()),"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "=dbo.fn_GetCurrentDateTime(GetDate()) And "
                                        End If
                                    End If
                                Case "TIME"
                                    If DefaultDR("Default_Value_Time").ToString.Trim <> "" Then
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'" & DefaultDR("Default_Value_Time").ToString.Trim & "',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='" & DefaultDR("Default_Value_Time").ToString.Trim & "' And "
                                        End If
                                    Else
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "dbo.fn_GetCurrentTime(GetDate()),"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "=dbo.fn_GetCurrentTime(GetDate()) And "
                                        End If
                                    End If
                                Case "OPTION"
                                    'Do nothing
                                ...
                                Case "CHARACTER"
                                    If DefaultDR("Default_Value_Character").ToString.Trim <> "" Then
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'" & DefaultDR("Default_Value_Character").ToString.Trim & "',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='" & DefaultDR("Default_Value_Character").ToString.Trim & "' And "
                                        End If
                                    Else
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='' And "
                                        End If
                                    End If
                                Case "INTEGER"
                                    If DefaultDR("Default_Value_Integer").ToString.Trim <> "" Then
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'" & DefaultDR("Default_Value_Integer").ToString.Trim & "',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='" & DefaultDR("Default_Value_Integer").ToString.Trim & "' And "
                                        End If
                                    Else
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'0',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='0' And "
                                        End If
                                    End If
                                Case "DECIMAL"
                                    If DefaultDR("Default_Value_Decimal").ToString.Trim <> "" Then
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'" & DefaultDR("Default_Value_Decimal").ToString.Trim & "',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='" & DefaultDR("Default_Value_Decimal").ToString.Trim & "' And "
                                        End If
                                    Else
                                        ssql4 = ssql4 & DefaultDR(0).ToString & ","
                                        ssql5 = ssql5 & "'0.00',"
                                        If DefaultDR(8).ToString = "YES" Then
                                            ssql6 = ssql6 & DefaultDR(0).ToString & "='0.00' And "
                                        End If
                                    End If
                            End Select
                            DefaultDR = Nothing
            ...
            If txtEFFECTIVE_DATE.Visible = False And ssql4.IndexOf("EFFECTIVE_DATE") < 0 _
                    And ssql5.IndexOf("EFFECTIVE_DATE") < 0 _
                    And ssql6.IndexOf("EFFECTIVE_DATE") < 0 Then
                ssql4 = ssql4 & "EFFECTIVE_DATE,"
                ssql5 = ssql5 & "dbo.fn_GetCurrentDate(GetDate()),"
                ssql6 = ssql6 & "EFFECTIVE_DATE=dbo.fn_GetCurrentDate(GetDate()) And "
            End If
            ssql4 = Left(ssql4, Len(ssql4) - 1) & Left(ssql5, Len(ssql5) - 1) & ")"
            ssql6 = Left(ssql6, Len(ssql6) - 5)
        End If

        myDS = New DataSet
        myDS = mySQL.ExecuteSQL(ssql6, Session("Company").ToString, Session("EmpID").ToString)
        If myDS.Tables(0).Rows.Count > 0 Then
            lblresult2.Text = "Duplicate Record Found!"
            Return False
        End If
        ...
        If Not myDS Is Nothing Then
            If myDS.Tables.Count > 0 Then
                ...
                Else
                    ...
                    Session("SearchSQL") = "Exec sp_Generate_Query_Filter_WithSecurity N'" & Session("Company").ToString & "',N'" & Session("EmpID").ToString & "',N'" & _
                            Session("Module").ToString & "',N'" & ssql & "',N'" & ssql2 & "',N'" & ssql3 & _
                            "','" & myGridView.PageSize & "','"
                    Session("SearchSQL1") = "Exec sp_Compare_Query_Filter_WithSecurity N'" & Session("Company").ToString & "',N'" & Session("EmpID").ToString & "',N'" & _
                            Session("Module").ToString & "',N'" & ssql & "',N'" & ssql2 & "',N'" & ssql3 & _
                            "','" & myGridView.PageSize & "','"
    ...
    Private Function ValidateUpdate() As Boolean
        lblresult2.Visible = True
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL("Select Code,Name,Option_Data_Type,Option_Mandatory From Table_Field Where Table_Profile_Code='" & Form.ID & "' And Option_Primary_Key='NO' And Option_View_Card='YES' Order By Table_Profile_Code,Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
        ssql = "Update " & Form.ID & " Set "
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                Select Case myDS.Tables(0).Rows(i).Item(2).ToString
                        ...
                        If myTextBox.Visible = True Then
                            ...
                            Else
                                If myDS.Tables(0).Rows(i).Item(3).ToString = "YES" Then
                                    ...
                                    Return False
                                Else
                                    ssql = ssql & myDS.Tables(0).Rows(i).Item(0) & "=N'" & myTextBox.Text.ToString.Trim & "',"
                                End If
                            End If
                        End If
                        myTextBox = Nothing
            ...
            myDS2 = New DataSet
            myDS2 = mySQL.ExecuteSQL("Select Code,Name,Option_Data_Type,Option_Mandatory From Table_Field Where Table_Profile_Code='" & Form.ID & "' And Option_Primary_Key='YES' And Option_View_Card='YES' Order By Table_Profile_Code,Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
            If myDS2.Tables(0).Rows.Count > 0 Then
                For Me.i = 0 To myDS2.Tables(0).Rows.Count - 1
                    Select Case myDS2.Tables(0).Rows(i).Item(2).ToString
                        Case "OPTION"
                        ...
                        Case "DATE"
                            ...
                            If myTextBox.Visible = True Then
                                If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                    If IsNumeric(mySetting.ConvertDateToDecimal(myTextBox.Text, Session("Company").ToString, Session("Module").ToString)) = True Then
                                        Dim myDST2 As New DataSet
                                        myDST2 = mySQL.ExecuteSQL("Select Function_Name,Default_Value From User_Define_Function Where Company_Profile_Code='" & Session("Company").ToString & "' And Module_Profile_Code='" & Session("Module").ToString & "' And Table_Profile_Code='" & Form.ID & "' And Table_Field_Code='" & myDS2.Tables(0).Rows(i).Item(0).ToString & "' And Query_Action='UPDATE'", Session("Company").ToString, Session("EmpID").ToString)
                            ...
                            If myTextBox.Visible = True Then
                                If mySetting.CheckTextNullAndReplaceSingleQuote(myTextBox) = True Then
                                    If myDS2.Tables(0).Rows(i).Item(0).ToString.ToUpper = "EMPLOYEE_PROFILE_ID" Then
                                        'Change for Performance when update start -- sytey
                                        myUpdate = mySQL.ExecuteSQL("select ISNULL(dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myTextBox.Text.ToString & "'),'')")
                                        If myUpdate.Tables(0).Rows.Count > 0 Then
                                            ssql = ssql & myDS2.Tables(0).Rows(i).Item(0) & "=N'" & myUpdate.Tables(0).Rows(0).Item(0).ToString & "' And "
                                        Else
                                            ssql = ssql & myDS2.Tables(0).Rows(i).Item(0) & "='' AND "
                                        End If
                                        myUpdate = Nothing
                                        'Change for Performance when update end -- sytey
                                    Else
                                        Dim myDST2 As New DataSet
...
#End Region
    ...
    Protected Sub ImgBtnAdd1_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgbtnAdd1.Click
    ...
    Private Function auto_Generate_EmpID(ByVal flag As Boolean) As String
        Dim dataCurrent As String = txtEMPLOYEE_PROFILE_ID.Text
        Dim strResult As String = ""
        If dataCurrent.Length = 0 Then
            myDS = mySQL.ExecuteSQL("select string_value1,string_value2,string_value3, string_value4 from parameter where company_profile_code='" & Session("Company") & "' and module_profile_code='" & Session("Module") & "' and code='EMPID_GENERATE'", Session("Company").ToString, Session("EmpID").ToString)
            If myDS.Tables.Count > 0 And myDS.Tables(0).Rows.Count > 0 And myDS.Tables(0).Columns.Count > 2 Then
                Dim dataFlag As String = myDS.Tables(0).Rows(0).Item(0).ToString
                Dim dataFormat As String = myDS.Tables(0).Rows(0).Item(1).ToString
                Dim dataStart As String = myDS.Tables(0).Rows(0).Item(2).ToString
                dataCurrent = myDS.Tables(0).Rows(0).Item(3).ToString
                If dataFlag = "YES" Then
                    If dataStart.Length = 0 Then
                        dataStart = "0"
                    End If
                    If dataCurrent.Length = 0 Then
                        dataCurrent = "0"
                    End If
                    strEmpCnt = dataCurrent + 1
                    Dim cnt As Integer = Convert.ToInt16(dataStart) + Convert.ToInt16(dataCurrent) + 1
                    Dim strCnt As String = cnt.ToString()
                    dataCurrent = strCnt
                    strEmpCnt = strCnt
                    Dim len As Integer = strCnt.Length()
                    Dim k As Integer = 0

                    For Me.i = dataFormat.Length - 1 To 0 Step -1
                        If dataFormat.Substring(i, 1) = "?" Then
                            If k < len Then
                                strResult = strCnt.Substring(len - k - 1, 1) + strResult
                            Else
                                strResult = "0" + strResult
                            End If
                            k = k + 1
                        Else
                            strResult = dataFormat.Substring(i, 1) + strResult
                        End If
                    Next
                End If
            End If
        End If
        If flag = True Then
            myDS = mySQL.ExecuteSQL("update parameter set string_value4 = '" + dataCurrent + "' from parameter where company_profile_code='" & Session("Company") & "' and module_profile_code='" & Session("Module") & "' and code='EMPID_GENERATE'", Session("Company").ToString, Session("EmpID").ToString)
        End If
        Return strResult
    End Function

    Protected Sub imgbtnPrevioPage_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnPrevioPage.Click
        Dim clsRpt As New clsReportServices, myTargetURL As String = ""
        myDS = mySQL.ExecuteSQL("select * from dbo.Wizard_Profile where Home = '" + Me.AppRelativeVirtualPath + "'", Session("Company").ToString)
        If myDS.Tables.Count > 0 And myDS.Tables(0).Rows.Count > 0 And myDS.Tables(0).Columns.Count > 2 Then
            Response.Redirect(myDS.Tables(0).Rows(0).Item(3).ToString)
        End If

    End Sub

    Protected Sub imgbtnNextPage_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnNextPage.Click
        Dim clsRpt As New clsReportServices, myTargetURL As String = ""
        myDS = mySQL.ExecuteSQL("select * from dbo.Wizard_Profile where Home = '" + Me.AppRelativeVirtualPath + "'", Session("Company").ToString)
        If myDS.Tables.Count > 0 And myDS.Tables(0).Rows.Count > 0 And myDS.Tables(0).Columns.Count > 2 Then
            Response.Redirect(myDS.Tables(0).Rows(0).Item(4).ToString)
        End If
    End Sub

    Protected Sub ImgBtnQuery_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgBtnQuery.Click
        For Me.i = 0 To myGridView.Rows.Count - 1
            Dim chkEdit As CheckBox = myGridView.Rows(i).Cells(0).Controls(1)
            If chkEdit.Checked Then
                chkEdit.Checked = False
                ...
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&Employee_Profile_ID=" & tmpEmpID & "&User_Profile_Code=" & Session("EmpID").ToString & "&rdReport=PersonalInfo"
                Response.Write("<script>window.open('" & myTargetURL & "');</script>")
            End If
        Next
