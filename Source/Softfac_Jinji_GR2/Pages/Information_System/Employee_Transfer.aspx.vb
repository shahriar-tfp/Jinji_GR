...
Partial Class Pages_InformationSystem_Employee_Transfer
...
#Region "Private Declaration"
    ...
    Dim ssqlTriggerUpdate As String
#End Region
...
#Region "Panel Edit"
    ...
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        If ValidateInsert() = True Then
            ssqlTriggerUpdate = "Exec sp_is_TriggerUpdateTransfer N'" & Session("Company").ToString & _
                    "',N'" & Session("EmpID").ToString & "',N'" & Session("Module").ToString & _
                    "',N'" & Form.ID.ToString & "',N'" & txtEMPLOYEE_PROFILE_ID.Text.ToString & _
                    "',N'" & ddlOPTION_TYPE.SelectedValue.ToString & "','" & txtVALUE_BEFORE_CHANGED.Text.ToString.Trim & _
                    "',N'" & txtVALUE_AFTER_CHANGED.Text.ToString.Trim & "',N'" & txtEFFECTIVE_DATE.Text.ToString.Trim & "'"
            Dim htb As New Hashtable

            htb.Add("ssql1", ssqlTriggerUpdate)
            htb.Add("ssql2", ssql)
            mySQL.ExecuteSQLTransactionByHashtable(htb, "Update Employee Transfer", Session("Company").ToString, Session("EmpID").ToString)
...
#Region "Panel Action"
    ...
    Protected Sub imgBtnEdit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnEdit.Click
        If ActionValidateEdit() Then
            ...
            ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "VALUE_BEFORE_CHANGED" & """," & """" & """," & """" & ddlOPTION_TYPE.SelectedValue & """"
            imgbtnVALUE_BEFORE_CHANGED.Attributes.Remove("onclick")
            mySetting.GetLookupValue_ImageButton(imgbtnVALUE_BEFORE_CHANGED, Form.ID, "txtVALUE_BEFORE_CHANGED", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "VALUE_BEFORE_CHANGED" & """," & """" & """,""" & Session("EmpID") & """,""" & ddlOPTION_TYPE.SelectedValue & """")
            imgbtnVALUE_AFTER_CHANGED.Attributes.Remove("onclick")
            mySetting.GetLookupValue_ImageButton(imgbtnVALUE_AFTER_CHANGED, Form.ID, "txtVALUE_AFTER_CHANGED", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "VALUE_AFTER_CHANGED" & """," & """" & """,""" & Session("EmpID") & """,""" & ddlOPTION_TYPE.SelectedValue & """")
        End If
    End Sub
...
#End Region
    ...
    Protected Sub ddlOPTION_TYPE_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlOPTION_TYPE.SelectedIndexChanged
        If Trim(txtEMPLOYEE_PROFILE_ID.Text) <> "" Then
            ssql = "Exec sp_sa_GetAutoFillData '" & Session("Company").ToString & _
                    "','" & Session("Module").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "','Employee_Transfer','EMPLOYEE_PROFILE_ID','AUTOFILL','" & ddlOPTION_TYPE.SelectedValue & "'"
            myDS = New DataSet
            myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            If Not myDS Is Nothing Then
                If myDS.Tables.Count > 0 Then
                    If myDS.Tables(0).Rows.Count > 0 Then
                        txtVALUE_BEFORE_CHANGED.Text = myDS.Tables(0).Rows(0).Item(0).ToString
                    End If
                End If
            End If
        End If
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "VALUE_BEFORE_CHANGED" & """," & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlOPTION_TYPE.SelectedValue & """"
        imgbtnVALUE_BEFORE_CHANGED.Attributes.Remove("onclick")
        mySetting.GetLookupValue_ImageButton(imgbtnVALUE_BEFORE_CHANGED, Form.ID, "txtVALUE_BEFORE_CHANGED", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "VALUE_BEFORE_CHANGED" & """," & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlOPTION_TYPE.SelectedValue & """")
        imgbtnVALUE_AFTER_CHANGED.Attributes.Remove("onclick")
        mySetting.GetLookupValue_ImageButton(imgbtnVALUE_AFTER_CHANGED, Form.ID, "txtVALUE_AFTER_CHANGED", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "VALUE_AFTER_CHANGED" & """," & """" & """," & """" & Session("EmpID").ToString & """," & """" & ddlOPTION_TYPE.SelectedValue & """")
        'imgbtnEMPLOYEE_PROFILE_ID.Attributes.Remove("onclick")
        'mySetting.GetLookupValue_ImageButton(imgbtnEMPLOYEE_PROFILE_ID, Form.ID, "txtEMPLOYEE_PROFILE_ID", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "EMPLOYEE_PROFILE_ID" & """," & """" & """," & """", ddlOPTION_TYPE.SelectedValue)
    End Sub
