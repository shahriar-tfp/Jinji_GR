...
Partial Class Pages_Approval_Medical_Approval
...
#Region "Private Declaration"
    ...
    Dim VisitDate, AutoApprovalNo, ClaimType, BENEFICIARYVISITER, BillNo, BillDate, BillAmt, PaymentMonth As String
                    ...
                    For Me.i = 0 To myGridView1.Rows.Count - 1
                        ...
                        If chkCursor.Checked = True Then
                            If chkCursor.Checked = True Then
                                ...
                                logic = True
                                While j < Count
                                    If myGridview2.Rows(i).Cells(3).Text = lstEmpID.Items(j).Text Then
                                        logic = False
                                    End If
                                    j = j + 1
                                End While
                                If logic = True Then
                                    lstEmpID.Items.Add(myGridview2.Rows(i).Cells(3).Text)
                                End If
                            End If
                        End If
                        chkCursor = Nothing
...
#Region "Sub & Function"
    ...
    Private Sub UpdPendingMedicalToAppMedical()
        ...
        For Me.i = 0 To myGridView1.Rows.Count - 1
            Dim chkCursor As CheckBox = myGridView1.Rows(i).Cells(0).Controls(1)
            If chkCursor.Checked = True Then
                VisitDate = Trim(myGridview2.Rows(i).Cells(2).Text)
                EmpID = Trim(myGridview2.Rows(i).Cells(3).Text)
                BillNo = Trim(myGridview2.Rows(i).Cells(5).Text)
                BillDate = Trim(myGridview2.Rows(i).Cells(6).Text)
                BillAmt = Trim(myGridview2.Rows(i).Cells(7).Text)
                PaymentMonth = Trim(myGridview2.Rows(i).Cells(8).Text)
                ClaimType = Trim(myGridview2.Rows(i).Cells(9).Text)
                BENEFICIARYVISITER = Trim(myGridview2.Rows(i).Cells(10).Text)
                AutoApprovalNo = Trim(myGridview2.Rows(i).Cells(11).Text)

                ssql = "Exec sp_md_updMedicalApproval '" & Session("Company") & "','" _
                       & VisitDate & "','" & EmpID & "','" _
                       & AutoApprovalNo & "','" & ClaimType & "','" & BENEFICIARYVISITER & "','" _
                       & BillNo & "','" & BillDate & "','" & BillAmt & "','" & PaymentMonth & "','" & "" & "','" & Session("EmpID") & "','" & "PAID" & "'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                myDS = Nothing
        ...
        For Me.i = 0 To myGridView1.Rows.Count - 1
            ...
            If chkCursor.Checked = True Then
                ...
                ssql = "Exec sp_md_updMedicalApproval '" & Session("Company") & "','" _
                       & VisitDate & "','" & EmpID & "','" _
                       & AutoApprovalNo & "','" & ClaimType & "','" & BENEFICIARYVISITER & "','" _
                       & BillNo & "','" & BillDate & "','" & BillAmt & "','" & PaymentMonth & "','" & "" & "','" & Session("EmpID") & "','" & "REJECT" & "'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                myDS = Nothing
...
#End Region
    ...
    Protected Sub lnkBtnCloseApproved_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkBtnCloseApproved.DataBinding

    End Sub

    Protected Sub imgBtnUpdate_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles imgBtnUpdate.Command

    End Sub

    Protected Sub Medical_Approval_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs) Handles Medical_Approval.DataBinding
