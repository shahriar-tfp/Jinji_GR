...
Partial Class Pages_Organisation_OCP_Code
        ...
        If ActionValidateDelete() Then
            ...
            For Me.i = 0 To myGridView.Rows.Count - 1
                ...
                If chkDelete.Checked Then
                    getnum += 1
                    ssql = mySetting.GetSQLParameter2(Form.ID, clsGlobalSetting.SQLAction.DELETE_Statement2, myGridView1, i, Session("Company"), Session("Module"))
                    If ssql <> Nothing Then
                        mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                        ssql = Nothing
                    End If
...
#Region "Sub & Function"
    ...
    Private Function ActionValidateEdit() As Boolean
        ...
        If RecFound = True Then
            If CountRecord > 1 Then
                lblresult.Text = "Invalid action: Only 1 row can be selected..."
                Return False
            Else
                Return True
            End If
    ...
    Private Function ValidateUpdate() As Boolean
        ...
        Try
            lblresult2.Visible = True
            myDS = mySQL.ExecuteSQL("Select Code,Option_Data_Type,[Name] From Table_Field Where Table_Profile_Code='" & "OCP_CODE" & "' And Option_Primary_Key='NO' And Option_View_Card='YES' Order By Table_Profile_Code,Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
            ssql = "Update [" & "OCP_CODE" & "] Set "
            If myDS.Tables(0).Rows.Count > 0 Then
                For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                myDS2 = mySQL.ExecuteSQL("Select Code,Option_Data_Type,[Name] From Table_Field Where Table_Profile_Code='" & "OCP_CODE" & "' And Option_Primary_Key='YES' Order By Table_Profile_Code,Sequence_No", Session("Company").ToString, Session("EmpID").ToString)
                If myDS2.Tables(0).Rows.Count > 0 Then
    ...
    Private Function ValidateInsert() As Boolean
        ...
        Try
            ...
            Dim dv As Integer = 7
            ssql = "exec sp_sa_get_insert_statement '" & Session("Company") & "','" & Session("Module") & "','" & "OCP_CODE" & "'"
            myDSInsert = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)

            lblresult2.Visible = True
            ssql = "Insert Into [" & "OCP_CODE" & "] ("
            ssql2 = ") Values("
            ssql3 = "Select * From [" & "OCP_CODE" & "] Where "
            ssql4 = ""
