...
Partial Class Pages_Organisation_Department_Section
...
#Region "Page Setting"
    ...
    Sub BindGrid()
        ...
        Try
            ...
            Else
                ssql = "Exec sp_sa_GetTableRecordsWithSecurity '" & Session("Company").ToString & "','" & Session("EmpID").ToString & "','" & Session("Module").ToString & "','" & "Department_Section_Grouping_Vw" & "','" & myGridView.PageSize & "','" & Session("currentpage") & "'"
                ssql1 = "exec sp_sa_compareTableRecords '" & Session("Company") & "','" & Session("EmpID") & "','" & Session("Module") & "','" & "Department_Section_Grouping_Vw" & "','" & myGridView.PageSize & "','" & Session("currentpage") & "'"
            End If
...
#Region "Panel Edit"
    ...
    Protected Sub imgBtnOCP_ID_Division_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnOCP_ID_Division.Click
        ...
        If CInt(Session("runNumber")) < CInt(Session("flowNumber")) And Session("action") <> "filter" Then
            If txtOCP_ID_SECTOR.Text = "" And Session("action") <> "filter" Then
...
#Region "Panel Action"
    ...
    Protected Sub imgBtnFilter_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnFilter.Click
        ...
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID_SECTOR" & """," & """" & """"
        mySetting.GetLookupValue_ImageButton(imgBtnOCP_ID_SECTOR, Form.ID, "txtOCP_ID_SECTOR", "CodeName", ssql)
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID_DIVISION" & """," & """" & """"
        mySetting.GetLookupValue_ImageButton(imgBtnOCP_ID_DIVISION, Form.ID, "txtOCP_ID_DIVISION", "CodeName", ssql)
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID_DEPARTMENT" & """," & """" & """"
        mySetting.GetLookupValue_ImageButton(imgBtnOCP_ID_DEPARTMENT, Form.ID, "txtOCP_ID_DEPARTMENT", "CodeName", ssql)
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "OCP_ID_SECTION" & """," & """" & """"
                    ...
                    Select Case myDS.Tables(0).Rows(i).Item(1).ToString
                        ...
                        Case "CHARACTER", "LOOKUP", "DECIMAL", "INTEGER"
                            If i = myDS.Tables(0).Rows.Count - 1 Then
                                ...
                                "' And Option_Type = 'Section' And [ID] In (Select OCP_ID_Section From Section_Line_Grouping where ocp_id_sector = '" & _
                                ...
                                mySetting.GetOCPIDbyCodeName_ReturnString(Session("Company"), txtOCP_ID_DEPARTMENT.Text) & "')"
