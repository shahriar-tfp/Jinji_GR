...
Partial Class Pages_Organisation_Employee_Group
...
#Region "Page Setting"
    ...
    Sub PagePreload()
        ...
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "EMPLOYEE_GROUP_ID" & """," & """" & """," & """" & Session("EmpID").ToString & """"
        mySetting.GetLookupValue_ImageButton(imgbtnEMPLOYEE_GROUP_ID, Form.ID, "txtEMPLOYEE_GROUP_ID", "CodeName", ssql)
        'ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "EMPLOYEE_PROFILE_ID" & """," & """" & """"
...
#Region "Panel Edit"
    ...
    Protected Sub imgbtnEMPLOYEE_PROFILE_ID_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnEMPLOYEE_PROFILE_ID.Click
        ...
        If ActionValidateFieldSecurityRoleHeader() = False Then
            lblresult2.Text = lblEMPLOYEE_GROUP_ID.Text & " field is required..."
            Exit Sub
        End If
        visibleLst()
        lstleft.Items.Clear()
    ...
    Protected Sub imgBtnEMPLOYEE_PROFILE_ID2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnEMPLOYEE_PROFILE_ID2.Click
        ...
        lstleft.Items.Clear()
        lstright.Items.Clear()
        ssql = "Exec sp_sa_getListBoxValue 'getEmpGroup','" & Session("Company") & "','" & txtEMPLOYEE_GROUP_ID.Text & "','','','','',''"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If myDS.Tables(0).Rows.Count > 0 Then
...
#Region "Sub & Function"
    ...
    Private Function ActionValidateFieldSecurityRoleHeader() As Boolean
        If txtEMPLOYEE_GROUP_ID.Text <> "" Then
            Return True
        Else
            Return False
        End If
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For Me.i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "EMPLOYEE_GROUP_ID" Then
                    If lblEMPLOYEE_GROUP_ID.Visible = True And txtEMPLOYEE_GROUP_ID.Visible = True Then
                        autonum += 1
