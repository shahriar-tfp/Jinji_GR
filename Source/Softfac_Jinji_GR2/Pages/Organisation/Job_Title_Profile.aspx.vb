...
Partial Class Pages_Organisation_Job_Title_Profile
...
#Region "Page Setting"
    ...
    Sub BindGrid()
        ...
        Try
            ...
            Else
                ssql = "Exec sp_sa_GetTableRecordsWithSecurity '" & Session("Company").ToString & "','" & Session("EmpID").ToString & "','" & Session("Module").ToString & "','" & "Job_Title_Profile_Vw" & "','" & myGridView.PageSize & "','" & Session("currentpage") & "'"
                ssql1 = "exec sp_sa_compareTableRecords '" & Session("Company") & "','" & Session("EmpID") & "','" & Session("Module") & "','" & "Job_Title_Profile_Vw" & "','" & myGridView.PageSize & "','" & Session("currentpage") & "'"
            End If
            ...
            If ValidateDelete() = True Then
                ...
                For Me.i = 0 To myGridView.Rows.Count - 1
                    ...
                    If chkDelete.Checked Then
                        getnum += 1
                        ssql = mySetting.GetSQLParameter2(Form.ID, clsGlobalSetting.SQLAction.DELETE_Statement3, myGridView, i, Session("Company"), Session("Module"))
                        If ssql <> Nothing Then
                            mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                            ssql = Nothing
                        End If
...
#Region "Sub & Function"
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        'This version only limited to strPosition type 2 only
        'anywhere can proceed to fulfill type 1 & 3.
        'And only support for 2 group objects calculation.
        '//-----------------------------------------------------------------------------------------//
    ...
    Private Function ValidateDelete() As Boolean
        ...
        For Me.i = 0 To myGridView.Rows.Count - 1
            ...
            If chkDelete.Checked Then
                ...
                ssql = "Select [Name] From Table_Field Where Table_Profile_Code ='Job_Title_Profile' And Code = 'OCP_ID_JOB_TITLE'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        ...
        ssql = "Select dbo.fn_GetOCPIDByCodeName('" & Trim(FieldValue) & "','" & "JOB_TITLE" & "')"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If Trim(myDS.Tables(0).Rows(0).Item(0).ToString) <> "" Then
        ...
        ssql = "Select * from Job_Grade_Job_Title_Grouping Where OCP_ID_Job_Title = '" & OCPIDCodeName & "'"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        ...
        ssql = "Select * from Employee_Status Where OCP_ID_Job_Title = '" & OCPIDCodeName & "'"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If Trim(myDS.Tables(0).Rows.Count) > 0 Then
            myDS = Nothing
            lblresult.Text = "Delete process failed! " & FieldValue & " currently is used..."
    ...
    Private Function ValidateUpdate() As Boolean
        ...
        Try
            ...
            If myDS.Tables(0).Rows.Count > 0 Then
                                ...
                                If myTextBox.Text <> "" Then
                                    If mySetting.ValidateInput(myTextBox, myDS.Tables(0).Rows(i).Item(1).ToString) = True Then
                                        ...
                                        If myDST.Tables(0).Rows.Count > 0 Then
                                            ...
                                            Else
                                                lblresult2.Text = "Process Terminated! Data Error for " & Trim(myDS.Tables(0).Rows(i).Item(2)) & "..."
                                                myDS1 = Nothing
                                                myDST = Nothing
                                                myDS = Nothing
                                                Return False
                ...
                myDS2 = Nothing
                If txtMin_Salary.Text = "" Then
                    txtMin_Salary.Text = 0
                End If
                If txtMax_Salary.Text = "" Then
                    txtMax_Salary.Text = 0
                End If
                If CDec(txtMin_Salary.Text) > CDec(txtMax_Salary.Text) Then
                    lblresult2.Text = "Value of " & lblMin_Salary.Text & " should NOT be greater then " & lblMax_Salary.Text & " value..."
                    Return False
                End If
            End If
            Return True
    ...
    Private Function ValidateInsert() As Boolean
        ...
        Try
            ...
            If myDSInsert.Tables(0).Rows.Count > 0 Then
                ...
                If imgMin_Salary.Visible = True And imgMax_Salary.Visible = True Then
                    If CDec(txtMin_Salary.Text) > CDec(txtMax_Salary.Text) Then
                        lblresult2.Text = "Value of " & lblMin_Salary.Text & " should NOT be greater then " & lblMax_Salary.Text & " value..."
                        Return False
                    End If
